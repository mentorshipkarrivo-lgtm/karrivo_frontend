...
// import {
//     User, Briefcase, Sparkles, X, ArrowLeft, ArrowRight, Check, CheckCircle2, Loader2, BookOpen
// } from 'lucide-react';
...
//     saving = false,
//     serverErrors = {},
//     mode = 'onboarding', // 'onboarding' | 'edit' | 'inline'
//     initialData = null,
//     onClose = null, // Required for modal modes
//     isOpen = true, // For modal modes
// }) {
...
//                 currentStatus: initialData.currentStatus || initialData.status || '',
//                 highestEducation: initialData.highestEducation || initialData.education || '',
...
//     const inputBase = "w-full px-3 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-400 transition-all";
...
//                             className={`${inputBase} ${errors.dateOfBirth ? 'border-red-400' : ''}`}
//                         />
...
//                                     className={`p-3 border-2 rounded-lg text-sm font-medium transition-all ${formData.currentStatus === s
...
//                             className={`${inputBase} ${errors.highestEducation ? 'border-red-400' : ''}`}
//                         >
...
//                                 className={`p-2.5 border-2 rounded-lg text-left transition-all ${formData.menteeType === type
//                                         ? 'border-blue-600 bg-blue-50 text-blue-700'
//                                         : 'border-gray-300 text-gray-700 hover:border-blue-400'
//                                     }`}
//                             >
...
//     // ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
//     // ONBOARDING MODE: Full-screen modal with steps
//     // ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
//     if (mode === 'onboarding') {
//         return (
...
//                     className="relative bg-white rounded-lg shadow-2xl w-full max-w-lg max-h-[90vh] overflow-hidden flex flex-col"
//                 >
...
//                                 <div
//                                     key={s}
//                                     className={`h-2 flex-1 rounded ${s <= currentStep ? 'bg-blue-600' : 'bg-gray-200'
//                                         }`}
//                                 />
//                             ))}
//                         </div>
...
//     // ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
//     // EDIT MODE: Side modal with animated steps (matches BookingsDashboard style)
//     // ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
//     if (mode === 'edit') {
...
//                                         <Fragment key={s.num}>
//                                             <div
//                                                 className={`flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-medium transition-all ${currentStep === s.num
...
//                                             {i < steps.length - 1 && (
//                                                 <div
//                                                     className={`flex-1 h-px ${currentStep > s.num ? 'bg-blue-200' : 'bg-slate-100'
//                                                         }`}
//                                                 />
//                                             )}
//                                         </Fragment>
...
//                                 <button
//                                     onClick={handleNext}
//                                     className="flex items-center gap-1.5 px-5 py-2 rounded-lg text-sm font-semibold text-white bg-blue-600 hover:bg-blue-700 transition-all"
//                                 >
...
import ProfileCompletionForm from "./Profilecompletionform"; // ← GLOBAL IMPORT
    ...
    return (
        ...
        >
            ...
            <div className="flex-1 min-w-0 px-4 py-3.5">
                ...
                <h3 className="text-sm font-semibold text-slate-900 truncate">{mentorName}</h3>
...
export default function BookingsDashboard() {
    ...
    const [isEditModalOpen, setEditModal] = useState(false); // ← USING FOR EDIT PREFERENCES MODAL
    ...
    useEffect(() => {
        ...
        let parsed = null;
        if (pc) {
            try { parsed = JSON.parse(decodeURIComponent(pc)); setProfileData(parsed); } catch { }
        }
        const uc = Cookies.get("userData");
        const mt = parsed?.menteeType || (uc ? JSON.parse(uc)?.menteeType : null) || "All Mentors";
        getMentors({ menteeType: mt });
    }, []);
    ...
    // ✅ GLOBAL PROFILE FORM CALLBACK
    const handlePreferencesUpdate = async (formData) => {
        try {
            setServerErrors({});
            const userId = localStorage.getItem('userData') ? JSON.parse(localStorage.getItem('userData')).username : null;
            ...
            if (userId && email) {
                ...
                setProfileData({ ...formData, profileCompleted: true });
                localStorage.setItem("profileData", JSON.stringify(formData));
                getMentors({ menteeType: formData.menteeType });
        ...
        <main className="h-[85vh] overflow-hidden bg-white font-[DM Sans,sans-serif]">
            ...
            <div className="h-full max-w-6xl mx-auto px-4 sm:px-6 py-6 flex flex-col lg:flex-row gap-6">
                ...
                <div className="flex-1 min-w-0 overflow-y-auto pr-1" style={{ scrollbarWidth: "thin", scrollbarColor: "#e2e8f0 transparent" }}>
                    ...
                    {/* ── Bookings list ── */}
                    {!bookingsLoading && !bookingsError && bookings.filter(b => b.paymentStatus === "Approved" || b.paymentStatus === "pending").length === 0 && (
                        <div className="border border-slate-200 rounded-xl p-4 mb-5 flex items-center justify-between bg-white">
                            <div>
                                <p className="text-sm font-semibold text-slate-900">No Upcoming Trial Bookings</p>
                                <p className="text-sm text-slate-500">Book one from mentors recommended for you</p>
                                <p className="text-xs text-slate-400 mt-1">Trials are 100% free and a great opportunity to finalise the mentor who's meant for you.</p>
                            </div>
                            <button className="flex-shrink-0 text-xs font-semibold text-slate-600 border border-slate-200 rounded-lg px-3 py-1.5 hover:bg-slate-50 transition-colors">
                                View All
                            </button>
                        </div>
                    )}
          ...
          ${panelOpen ? "opacity-100 pointer-events-auto" : "opacity-0 pointer-events-none"}`}
            ...
            {/* ✅ USING GLOBAL PROFILE FORM IN EDIT MODE */}
