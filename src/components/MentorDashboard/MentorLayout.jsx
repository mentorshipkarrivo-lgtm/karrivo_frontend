...
// ═══════════════════════════════════════════════════════════════════════════════
// EditMentorProfile Modal (unchanged from original)
// ═══════════════════════════════════════════════════════════════════════════════
// function EditMentorProfile({ onClose, initialTab = "overview" }) {
//     const [formData, setFormData] = useState({ availability: [] });
//     const [email, setEmail] = useState("");
//     const [modalTab, setModalTab] = useState(initialTab);  // ← CHANGE THIS LINE
...
//     useEffect(() => { if (data?.data) { serverRef.current = data.data; setFormData({ ...data.data }); } }, [data]);
//     useEffect(() => { if (formData.profilePhoto && photoStatus === "idle") setPhotoPreview(formData.profilePhoto); }, [formData.profilePhoto]);
...
//     // IN useEffect to set initialTab when it changes:
//     useEffect(() => {
//         if (initialTab) {
...
//     const availRemoveSlot = (ds, startTime) => setFormData((p) => ({ ...p, availability: (p.availability || []).filter((s) => !(new Date(s.date).toISOString().split("T")[0] === ds && s.startTime === startTime)) }));
...
//             <div className="edit-modal" style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.6)", zIndex: 9999, display: "flex", alignItems: "center", justifyContent: "center", padding: "20px 60px" }}>
//                 <div className="modal-card" style={{ background: T.bg, width: "100%", maxWidth: 820, maxHeight: "92vh", display: "flex", flexDirection: "column", boxShadow: "0 32px 100px rgba(0,0,0,0.3)", border: `1px solid ${T.border}`, borderRadius: 18, overflow: "hidden" }}>
...
//                         <div style={{ background: T.surface, borderRight: `1px solid ${T.border}`, width: 180, flexShrink: 0, display: "flex", flexDirection: "column" }}>
//                             <nav style={{ display: "flex", flexDirection: "column", gap: 3, padding: "14px 10px", flex: 1, overflow: "auto" }}>
//                                 {modalNav.map(({ id, label, icon: Icon }) => (
//                                     <button key={id} type="button" onClick={() => { setModalTab(id); setModalErrors({}); }}
//                                         style={{ display: "flex", alignItems: "center", gap: 8, padding: "10px 12px", borderRadius: 9, fontSize: 12.5, fontWeight: 700, fontFamily: F, cursor: "pointer", whiteSpace: "nowrap", background: modalTab === id ? T.btn : "transparent", color: modalTab === id ? "#fff" : T.textMid, border: `1px solid ${modalTab === id ? T.btn : "transparent"}`, transition: "all .15s" }}>
//                                         <Icon size={14} />{label}
//                                         {modalTab === id && <ChevronRight size={12} style={{ marginLeft: "auto" }} />}
//                                     </button>
//                                 ))}
//                             </nav>
...
//             {/* ── Overlay ── */}
//             <div className="edit-modal em-overlay" style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.6)", zIndex: 9999, display: "flex" }}>
...
const RightPanel = () => {
    ...
    return (
        <div className="right-panel" style={{ width: 296, flexShrink: 0, borderLeft: `1px solid ${T.border}`, background: "#fff", display: "flex", flexDirection: "column", overflowY: "auto" }}>
...
const CenterContent = ({ children, isHome, onEditProfileOpen, onSetEditTab }) => {
    ...
    if (!isHome) return <div style={{ flex: 1, padding: "12px 12px", overflowY: "auto", background: "#fff" }}>{children}</div>;
