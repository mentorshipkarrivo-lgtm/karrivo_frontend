...
const Toggle = ({ on, onChange }) => (
  <button
    type="button"
    onClick={() => onChange(!on)}
    className={`relative w-9 h-5 rounded-full transition-colors flex-shrink-0 focus:outline-none focus:ring-2 focus:ring-offset-1 focus:ring-[#1a1a2e]/30 ${on ? "bg-[#1a1a2e]" : "bg-slate-200"
      }`}
  >
    <span
      className={`absolute top-0.5 w-4 h-4 rounded-full bg-white shadow-sm transition-all duration-200 ${on ? "left-[18px]" : "left-0.5"
...
const Badge = ({ children, variant = "default" }) => {
  const cls = {
    default: "bg-slate-100 border-slate-200 text-[#94a3b8]",
    active: "bg-red-50 border-red-200 text-red-500",
    upcoming: "bg-amber-50 border-amber-200 text-amber-600",
    success: "bg-[#1a1a2e]/8 border-[#1a1a2e]/20 text-[#1a1a2e]",
  }[variant];
  return (
    <span className={`inline-flex items-center text-[10px] font-semibold px-2 py-0.5 rounded-full border ${cls}`}>
      {children}
...
const CardHeader = ({ title, icon: Icon, right }) => (
  <div className="flex items-center justify-between px-4 py-3 bg-slate-50 border-b border-slate-200">
    <div className="flex items-center gap-2">
      {Icon && <Icon size={14} className="text-[#1a1a2e]" />}
      <span className="text-xs font-semibold text-slate-700 tracking-wide">{title}</span>
    </div>
    {right && <div>{right}</div>}
  </div>
...
const inputCls =
  "w-full bg-white border border-slate-200 rounded-lg px-3 py-2 text-sm text-slate-800 outline-none focus:border-[#1a1a2e] focus:ring-2 focus:ring-[#1a1a2e]/10 transition placeholder:text-[#94a3b8] appearance-none";
const inputErrCls =
  "w-full bg-white border border-red-300 rounded-lg px-3 py-2 text-sm text-slate-800 outline-none focus:border-red-400 focus:ring-2 focus:ring-red-400/10 transition placeholder:text-[#94a3b8] appearance-none";

/* Button styles using #1a1a2e as primary */
const btnPrimary =
  "inline-flex items-center gap-1.5 bg-[#1a1a2e] hover:bg-[#2d2d4e] text-white text-sm font-semibold px-4 py-2 rounded-lg transition disabled:opacity-40 disabled:cursor-not-allowed";
const btnOutline =
  "inline-flex items-center gap-1.5 border border-slate-200 hover:border-[#1a1a2e]/30 hover:bg-[#1a1a2e]/4 text-[#94a3b8] hover:text-[#1a1a2e] text-sm font-semibold px-4 py-2 rounded-lg transition";
const btnDanger =
  "inline-flex items-center gap-1.5 border border-red-200 hover:bg-red-50 text-red-400 hover:text-red-500 text-xs font-semibold px-3 py-1.5 rounded-lg transition";
...
export default function MentorAvailability() {
  ...
  return (
    <div className="min-h-screen">
      ...
      {/* ── Toast ── */}
      {toastMsg && (
        <div className="fixed top-4 left-1/2 -translate-x-1/2 z-[60] flex items-center gap-2 bg-[#1a1a2e] border border-[#1a1a2e]/80 text-white text-xs font-semibold px-4 py-2.5 rounded-xl shadow-lg anim-fade">
          ...
          <div className="flex gap-1 mt-4">
            {[
              ...
              >
                ...
                {label}
                {key === "unavailability" && unavailRecs.length > 0 && (
                  <span className="text-[9px] bg-slate-100 border border-slate-200 px-1.5 py-0.5 rounded-full font-bold text-[#94a3b8]">
                    {unavailRecs.length}
                  </span>
                )}
              </button>
      ...
      <div className="max-w-3xl mx-auto px-4 py-5">
              ...
              <div className="anim-slide">
                <Card>
                  ...
                  <div className="p-4">
                      ...
                      {ALL_DAYS.map((day) => {
                        ...
                        return (
                          ...
                          >
                            ...
                            {slot?.enabled && !block && (
                              <>
                                <Clock size={10} className="text-[#94a3b8] flex-shrink-0" />
                                <span className="text-slate-700">
                                  {slot.from} – {slot.to}
                                </span>
                              </>
                            ...
                            {!slot?.enabled && (
                              <span className="text-[#94a3b8] italic text-[11px]">Rest day</span>
                            )}
                          </div>
                        );
                      })}
                    ...
                    <div className="flex flex-wrap gap-x-4 gap-y-1.5 pt-3 border-t border-slate-100 text-xs text-[#94a3b8]">
                      <span>
                        <strong className="text-[#1a1a2e] font-semibold">{spw}</strong>{" "}
                        session{spw > 1 ? "s" : ""}/week
                      </span>
                      <span className="text-slate-300">·</span>
                      <span>
                      ...
                      {notes && (
                        <>
                          <span className="text-slate-300">·</span>
                          <span className="italic text-[#94a3b8]">{notes}</span>
                        </>
                      )}
                    </div>
                  </div>
                  ...
                  <div className="p-4 space-y-2">
                    {daySlots.map((slot, i) => (
                      ...
                      >
                        ...
                        />
                        <span
                          className={`text-[10px] font-mono font-bold uppercase w-7 ${slot.enabled ? "text-[#1a1a2e]" : "text-slate-300"
                            }`}
                        >
                          {slot.day}
                        </span>
                          ...
                          <div className="flex flex-wrap items-center gap-2 flex-1 min-w-0">
                            {[
                              ...
                              <div key={f} className="flex items-center gap-1">
                                ...
                                <select
                                  ...
                                  className="bg-white border border-slate-200 text-[#1a1a2e] rounded-lg px-2 py-1 text-[11px] font-mono font-semibold outline-none focus:border-[#1a1a2e] transition cursor-pointer"
                                >
                                  {TIMES.map((t) => (
                                    <option key={t} value={t}>
                                      {t}
                            ...
                            <div className="ml-auto flex gap-1">
                              <button
                                ...
                                className="w-7 h-7 flex items-center justify-center rounded-lg bg-[#1a1a2e]/6 border border-[#1a1a2e]/15 hover:bg-[#1a1a2e]/12 transition"
                                title="Add slot"
                              >
                                <Plus size={11} className="text-[#1a1a2e]" />
                              </button>
                              <button
                                type="button"
                                onClick={() =>
        ...
        {tab === "unavailability" && (
          <div className="anim-slide">
            <div className="flex items-start gap-2.5 mb-4 bg-[#1a1a2e]/4 border border-[#1a1a2e]/12 rounded-xl px-4 py-3 text-xs text-[#1a1a2e]/80">
              <Info size={13} className="text-[#1a1a2e]/60 flex-shrink-0 mt-0.5" />
              <p>
                Blocking a date range automatically extends all affected mentee subscriptions
                by the exact working days lost — so they always receive their full plan.
              </p>
            ...
            {!unavailRecs.length ? (
              <div className="bg-white border border-dashed border-slate-200 rounded-xl p-14 text-center">
                <div className="w-12 h-12 rounded-xl bg-[#1a1a2e]/6 flex items-center justify-center mx-auto mb-3">
                  <CheckCircle2 size={22} className="text-[#1a1a2e]/30" />
                </div>
                <p className="font-semibold text-slate-800 text-sm">No blocked periods</p>
                <p className="text-xs text-[#94a3b8] mt-1">All your days are currently open.</p>
              </div>
            ) : (
              <div className="space-y-3">
                {unavailRecs.map((u) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="p-4">
                        ...
                        {u.notes && (
                          <p className="text-xs text-[#94a3b8] italic mt-2.5">{u.notes}</p>
                        )}
                      </div>
                    </div>
                  );
        ...
        <div className="fixed inset-0 z-50 bg-black/40 backdrop-blur-sm flex items-center justify-center p-4 overflow-y-auto">
          <div className="bg-white border border-slate-200 rounded-2xl shadow-2xl w-full max-w-md my-4 anim-fade">
            ...
            <div className="flex items-center justify-between px-5 py-4 border-b border-slate-200 bg-slate-50 rounded-t-2xl">
              ...
              <button
                onClick={() => setShowModal(false)}
                className="text-[#94a3b8] hover:text-slate-700 transition p-1 rounded-lg hover:bg-slate-100"
              >
                <X size={16} />
              </button>
            </div>
