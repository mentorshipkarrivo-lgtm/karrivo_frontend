...
//                 style={{
//                   fontFamily: F, fontSize: 12, border: '1.5px solid rgba(0,152,204,0.26)',
//                   borderRadius: 7, padding: '8px 10px', color: '#e8f5f0',
//                   background: 'rgba(0,0,0,0.3)', outline: 'none', colorScheme: 'dark'
//                 }}
//               />
...
//       gap: 16, padding: '20px 24px', borderBottom: '1px solid rgba(0,152,204,0.14)'
//     }}>
//       <div style={{ display: 'flex', alignItems: 'center', gap: 10, minWidth: 0 }}>
//         {Icon && (
//           <div style={{
...
//           {subtitle && <p style={{ fontFamily: F, fontSize: 12, color: '#4a8070', margin: '2px 0 0', lineHeight: 1.5 }}>{subtitle}</p>}
//         </div>
//       </div>
//       <button
...
//     </div>
//     {href ? (
//       <a href={href} target="_blank" rel="noopener noreferrer" style={{ fontFamily: F, fontSize: 12.5, color: '#0098cc', fontWeight: 500, textDecoration: 'none', display: 'flex', alignItems: 'center', gap: 4 }}>
...
//   // Only sync `cur` when modal freshly opens (false → true).
//   // Never list `section` in deps to avoid resetting cur mid-edit.
//   useEffect(() => {
//     if (isOpen && !wasOpenRef.current) {
//       setCur(section);
//       setErrors({});
...
//   // "Save" button — save and close modal
//   const handleSaveAndClose = async () => {
//     if (!validate()) return;
//     await onSave(true); // true = close after save
//   };

//   // "Save & Continue" button — save, stay open, advance to next tab
...
//       setErrors({});
//     }
//     await onSave(false); // false = don't close
//   };
...
//                 <span style={{ fontFamily: F, fontSize: 12.5, color: '#8fbfb0', flex: 1 }}>{a}</span>
//                 <button
...
//       <div
//         style={{
//           background: '#0a2d1e', width: '100%', maxWidth: 760, maxHeight: '92vh',
//           display: 'flex', flexDirection: 'column',
//           boxShadow: '0 24px 80px rgba(0,0,0,0.6)', border: '1px solid rgba(0,152,204,0.26)'
//         }}
//         className="mp-modal"
//       >
//         {/* Header */}
...
//           padding: '16px 20px', borderBottom: '1px solid rgba(0,152,204,0.14)', flexShrink: 0
//         }}>
...
//         <div style={{ display: 'flex', flex: 1, minHeight: 0, flexDirection: 'column' }} className="mp-modal-body">
//           {/* Nav */}
//           <div style={{ background: 'rgba(0,0,0,0.2)', borderBottom: '1px solid rgba(0,152,204,0.14)', flexShrink: 0 }} className="mp-nav">
//             <nav style={{ display: 'flex', gap: 4, padding: '10px 14px', overflowX: 'auto' }}>
//               {navItems.map(({ id, label, icon: Icon }) => (
//                 <button
//                   key={id} type="button"
...
//           borderTop: '1px solid rgba(0,152,204,0.14)',
...
//             >
//               {isSaving
//                 ? <><Loader2 size={12} style={{ animation: 'spin .9s linear infinite' }} />Saving…</>
//                 : 'Save'
//               }
//             </button>

//             {/* Save & Continue — only on non-last tabs; saves and advances tab */}
//             {!isLastTab && (
//               <button
...
//                   cursor: 'pointer', opacity: isSaving ? .6 : 1
//                 }}
//               >
//                 {isSaving
//                   ? <><Loader2 size={12} style={{ animation: 'spin .9s linear infinite' }} />Saving…</>
...
//   /**
//    * shouldClose = true  → called from "Save" button  → save + close modal
//    * shouldClose = false → called from "Save & Continue" → save, modal stays open
//    */
...
//       <div style={{ textAlign: 'center' }}>
//         <Loader2 size={28} style={{ color: '#0098cc', animation: 'spin .9s linear infinite', display: 'block', margin: '0 auto 12px' }} />
//         <p style={{ fontFamily: F, color: '#4a8070', fontSize: 13, margin: 0 }}>Loading profile…</p>
//       </div>
//     </div>
//   );
...
//       <div style={{ background: '#0a2d1e', borderRadius: 16, border: '1px solid rgba(0,152,204,0.14)', padding: '36px 40px', maxWidth: 340, width: '100%', textAlign: 'center' }}>
//         <UserX size={32} style={{ color: '#8fbfb0', display: 'block', margin: '0 auto 12px' }} />
//         <p style={{ fontFamily: F, color: '#8fbfb0', fontSize: 14, fontWeight: 700, margin: 0 }}>No Profile Found</p>
//       </div>
//     </div>
//   );
...
//           .mp-modal-overlay { align-items:center; padding:20px; }
//           .mp-modal-body { flex-direction:row !important; }
//           .mp-nav { border-bottom:none !important; border-right:1px solid rgba(0,152,204,0.14); width:180px; flex-shrink:0; }
//           .mp-nav nav { flex-direction:column !important; padding:12px; gap:4px; }
//         }
//         @media (max-width: 900px) {
//           .mp-main-grid { grid-template-columns:1fr !important; }
...
//           {/* Hero */}
//           <div style={{
//             background: 'linear-gradient(135deg,#071e12 0%,#0a2a18 50%,#062117 100%)',
//             border: '1px solid rgba(0,152,204,0.14)', borderRadius: 16,
//             padding: '28px 28px 24px', marginBottom: 20, animation: 'fadeUp .3s ease both'
//           }}>
//             <div style={{ display: 'flex', flexWrap: 'wrap', alignItems: 'center', gap: 20, justifyContent: 'space-between' }}>
//               <div style={{ display: 'flex', alignItems: 'center', gap: 18, minWidth: 0 }}>
//                 <div style={{ position: 'relative', flexShrink: 0 }}>
...
//                       style={{ width: 72, height: 72, borderRadius: 14, objectFit: 'cover', border: '2px solid rgba(0,152,204,0.26)', display: 'block' }}
//                       onError={e => e.target.style.display = 'none'}
//                     />
//                   ) : (
//                     <div style={{
//                       width: 72, height: 72, borderRadius: 14,
//                       background: 'rgba(0,152,204,0.12)', border: '2px solid rgba(0,152,204,0.26)',
//                       display: 'flex', alignItems: 'center', justifyContent: 'center',
//                       fontSize: 24, fontWeight: 800, color: '#0098cc', fontFamily: F
//                     }}>{initials}</div>
//                   )}
//                   {formData.status === 'approved' && (
//                     <div style={{
//                       position: 'absolute', bottom: -4, right: -4, width: 18, height: 18,
//                       borderRadius: '50%', background: '#22c55e', border: '2px solid #062117',
//                       display: 'flex', alignItems: 'center', justifyContent: 'center'
//                     }}>
//                       <CheckCircle size={10} color="#fff" />
//                     </div>
...
//                       <span style={{
//                         fontFamily: F, fontSize: 9.5, fontWeight: 700, color: '#0098cc', textTransform: 'uppercase',
//                         letterSpacing: '.5px', background: 'rgba(0,152,204,0.12)', border: '1px solid rgba(0,152,204,0.26)',
//                         padding: '2px 9px', borderRadius: 20
//                       }}>{formData.mentorCategory}</span>
//                     )}
//                     <span style={{
//                       fontFamily: F, fontSize: 9.5, fontWeight: 700, color: '#e8a020',
//                       background: 'rgba(232,160,32,0.12)', border: '1px solid rgba(232,160,32,0.25)',
//                       padding: '2px 9px', borderRadius: 20
//                     }}>★ {formData.rating || '5.0'}</span>
//                     {formData.status === 'approved' && (
//                       <span style={{
//                         fontFamily: F, fontSize: 9.5, fontWeight: 700, color: '#22c55e',
//                         background: 'rgba(34,197,94,0.08)', border: '1px solid rgba(34,197,94,0.2)',
//                         padding: '2px 9px', borderRadius: 20
//                       }}>✓ Verified</span>
//                     )}
//                   </div>
//                   <h1 style={{ fontFamily: F, fontSize: 'clamp(18px,3vw,24px)', fontWeight: 800, color: '#e8f5f0', margin: '0 0 3px', lineHeight: 1.2 }}>
//                     {formData.fullName || <span style={{ color: '#4a8070', fontStyle: 'italic', fontWeight: 500 }}>No name added</span>}
//                   </h1>
//                   <p style={{ fontFamily: F, fontSize: 13, color: '#8fbfb0', margin: '0 0 8px', fontWeight: 500 }}>
//                     {formData.currentRole || 'Mentor'}
//                     {formData.companyName && <span style={{ color: '#4a8070' }}> · {formData.companyName}</span>}
//                   </p>
//                   <div className="mp-hero-meta" style={{ display: 'flex', flexWrap: 'wrap', gap: 14 }}>
//                     {formData.location && (
//                       <span style={{ fontFamily: F, fontSize: 11.5, color: '#4a8070', display: 'flex', alignItems: 'center', gap: 4 }}>
...
//                     )}
//                     {formData.createdAt && (
//                       <span style={{ fontFamily: F, fontSize: 11, color: '#4a8070', display: 'flex', alignItems: 'center', gap: 4 }}>
//                         <Clock size={10} />Since {fmtDate(formData.createdAt)}
//                       </span>
//                     )}
//                   </div>
//                 </div>
...
//               <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', alignItems: 'center' }}>
//                 <div style={{ background: 'rgba(0,152,204,0.12)', border: '1px solid rgba(0,152,204,0.26)', borderRadius: 10, padding: '10px 14px', textAlign: 'center', minWidth: 70 }}>
...
//                 </div>
//                 <div style={{ background: 'rgba(0,152,204,0.12)', border: '1px solid rgba(0,152,204,0.26)', borderRadius: 10, padding: '10px 16px', display: 'flex', alignItems: 'baseline', gap: 5 }}>
...
//                   style={{ background: '#0098cc', border: 'none', borderRadius: 10, padding: '10px 16px', fontFamily: F, fontSize: 12, fontWeight: 700, color: '#fff', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 6 }}
//                 >
//                   <Edit size={13} />Edit Profile
//                 </button>
//               </div>
...
//                     <p style={{ fontFamily: F, fontSize: 13, color: '#8fbfb0', lineHeight: 1.8, margin: 0 }}>{bio}</p>
//                   </div>
//                 )}
//                 <div style={{ display: 'flex', flexDirection: 'column', gap: 8, marginBottom: skills.length ? 18 : 0 }}>
//                   {formData.email && <InfoRow icon={Mail}>{formData.email}</InfoRow>}
...
//                         <p style={{ fontFamily: F, fontSize: 13, fontWeight: 700, color: '#e8f5f0', margin: 0 }}>{item.value}</p>
//                       </div>
//                     ))}
//                   </div>
//                 )}
...
//                             <Trophy size={13} color="#e8a020" style={{ flexShrink: 0, marginTop: 1 }} />
//                             <span style={{ fontFamily: F, fontSize: 12.5, color: '#8fbfb0' }}>{a}</span>
//                           </div>
//                         ))}
//                       </div>
//                     </div>
...
//                             style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '10px 12px', background: 'rgba(0,0,0,0.15)', border: '1px solid rgba(0,152,204,0.14)', borderRadius: 9, textDecoration: 'none' }}
//                           >
...
//                               <Icon size={12} color="#0098cc" />
//                             </div>
//                             <span style={{ fontFamily: F, fontSize: 12, fontWeight: 600, color: '#8fbfb0' }}>{label}</span>
//                             <ExternalLink size={10} color="#4a8070" style={{ marginLeft: 'auto' }} />
//                           </a>
//                         ))}
//                       </div>
//                     </div>
