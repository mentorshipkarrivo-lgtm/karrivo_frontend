...
//                     currentRole: formData.currentRole === 'Other' ? formData.customRole : formData.currentRole
//                 };
...
//                 <SuccessScreen />
//             ) : (
//                 <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-50 dark:from-slate-900 dark:via-slate-800 dark:to-slate-900 py-6 pt-20 px-4">                    <div className="max-w-4xl mx-auto">

//                     {/* Header */}
//                     <div className="bg-white dark:bg-slate-800 rounded-xl p-4 mb-5 shadow-sm border border-gray-100 dark:border-slate-700">
...
//                                             <div className={`w-10 h-10 rounded-lg flex items-center justify-center font-bold transition-all ${isCompleted ? 'bg-[#0098cc] text-white' : isCurrent ? 'bg-[#0098cc] text-white ring-2 ring-blue-200' : 'bg-gray-100 dark:bg-slate-700 text-gray-400 dark:text-gray-500'}`}>
//                                                 {isCompleted ? <Check size={18} strokeWidth={3} /> : <Icon size={18} />}
//                                             </div>
//                                             <div className={`text-xs font-semibold mt-1.5 ${isCurrent || isCompleted ? 'text-[#0098cc]' : 'text-gray-500 dark:text-gray-400'}`}>
//                                                 {step.name}
//                                             </div>
//                                         </div>
...
//                                                 disabled={isLoading}
//                                                 className="px-2 py-2 text-sm border border-gray-300 rounded-lg focus:outline-none focus:ring-1 focus:ring-[#0098cc] bg-white appearance-none cursor-pointer disabled:bg-gray-50 disabled:cursor-not-allowed"
//                                                 style={{ minWidth: 110 }}
//                                             >
...
//                                                     disabled={isLoading}
//                                                     className={`w-full pl-9 pr-3 py-2 text-sm border rounded-lg focus:outline-none focus:ring-1 focus:ring-[#0098cc] focus:border-[#0098cc] transition-all disabled:bg-gray-50 disabled:cursor-not-allowed ${errors.phone ? 'border-red-400 bg-red-50' : 'border-gray-300 hover:border-gray-400'}`}
//                                                 />
//                                             </div>
//                                         </div>
//                                         {errors.phone && (
    ...
    const validateSection = (section) => {
        ...
        if (section === 1) {
            ...
            if (formData.phone.trim()) {
                const localNumber = formData.phone.trim();
                const digitsOnly = localNumber.replace(/\D/g, '');
                const phoneLengthMap = { '+91': 10, '+1': 10, '+44': 10, '+61': 9, '+971': 9, '+65': 8, '+353': 9 };
                const expectedLength = phoneLengthMap[phoneCode];
                if (!/^\d+$/.test(localNumber)) { newErrors.phone = 'Phone number must contain digits only'; }
                else if (expectedLength && digitsOnly.length !== expectedLength) { newErrors.phone = `Phone number must be exactly ${expectedLength} digits for ${phoneCode}`; }
                else if (!expectedLength && (digitsOnly.length < 7 || digitsOnly.length > 15)) { newErrors.phone = 'Phone number must be between 7 and 15 digits'; }
            }
                            ...
                            {currentStep === 1 && (
                                <div>
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                        ...
                                        <div className="mb-4">
                                            <label className="block text-xs font-semibold text-gray-700 dark:text-white mb-1.5">Phone Number</label>
                                            <div className="flex gap-2">
                                                <select
                                                    value={phoneCode}
                                                    onChange={e => setPhoneCode(e.target.value)}
                                                ...
                                                <div className="relative flex-1">
                                                    <div className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400 dark:text-gray-300"><Phone size={16} /></div>
                                                    <input
                                                        type="tel"
                                                        name="phone"
                                                        value={formData.phone || ''}
                                                        ...
                                                        placeholder="9876543210"
                                                        disabled={isLoading}
                                                        className={`${inputBase} pl-9 ${errors.phone ? 'border-red-400 dark:border-red-500 bg-red-50 dark:bg-red-900/20' : ''}`}
                                                    />
                                                </div>
                                            </div>
                                            {errors.phone && <p className="text-red-500 text-xs mt-1 flex items-center gap-1"><span className="inline-block w-1 h-1 bg-red-500 rounded-full" />{errors.phone}</p>}
                                        </div>
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                        <div className="md:col-span-2 mb-2">
                                            ...
                                            <select
                                                ...
                                                onChange={handleInputChange}
                                                disabled={isLoading}
                                                className={`${selectBase} ${errors.currentRole ? 'border-red-400 dark:border-red-500' : ''}`}
                                            >
                                                <option value="">Select your current role</option>
                                                {suggestedRoles.map(role => <option key={role} value={role}>{role}</option>)}
                                            </select>
                                            ...
                                            {formData.currentRole === 'Other' && (
                                                <div className="mt-3">
                                                    <input
                                                        ...
                                                        placeholder="Please specify your role"
                                                        disabled={isLoading}
                                                        className={`${inputBase} pl-3 ${errors.customRole ? 'border-red-400 dark:border-red-500 bg-red-50 dark:bg-red-900/20' : ''}`}
                                                    />
                                                    {errors.customRole && <p className="text-red-500 text-xs mt-1 flex items-center gap-1"><span className="inline-block w-1 h-1 bg-red-500 rounded-full" />{errors.customRole}</p>}
                                                </div>
                                            )}
                                        </div>
                                        {renderInput('Company / Organization', 'companyName', 'text', true, 'e.g., Google, Microsoft')}
