...
// const SectionDivider = ({ label, collapsed }) => (
//     <div className={`pt-3 pb-1 ${collapsed ? 'flex justify-center' : 'px-3'}`}>
//         {collapsed
...
//             {/* <SectionDivider
//                 label={<span className="font-bold">Long-Term Mentorship</span>}
...
// ✅ Check if profile is already completed — looks in localStorage AND cookies
...
// ✅ Save profileData to both localStorage and cookie
...
const ProfileCompletionForm = ({ onComplete }) => {
    ...
    const submit = () => {
        const d = { ...formData, profileCompleted: true };
        saveProfileData(d); // ✅ saves to both localStorage + cookie
        onComplete(d);
    };
    return (
        <div className="flex items-center justify-center min-h-screen p-4">
...
const MenteeDashboard = () => {
    ...
    useEffect(() => {
        // ── Load user data (localStorage first, fallback to cookie) ──
        ...
        // ✅ Only show onboarding if profileCompleted is NOT already saved
