...
const hoverBtnIn = (e) => { e.currentTarget.style.filter = "brightness(0.92)"; };
const hoverBtnOut = (e) => { e.currentTarget.style.filter = "none"; };

/* ─── Contact items config ─── */
const CONTACT_ITEMS = [
    { key: "wa", iconSize: 16, label: "WhatsApp", desc: "Chat with us instantly", bg: "#edfcf2", color: "#25D366", external: true },
    { key: "call", iconSize: 16, label: "Call us", desc: "Speak to our team directly", bg: "#eef8fc", color: "#0098cc", external: true },
    { key: "email", iconSize: 16, label: "Email", desc: "Get a detailed response", bg: "#eef8fc", color: "#0098cc", external: true },
];

const HELP_ITEMS = [
    // { key: "faq", label: "FAQs", desc: "Quick answers to common questions" },
    ...
    return (
        <div className="mentee-support-widget" style={{
            ...
            <div style={{
                ...
                {view !== "home" && (
                    <button onClick={goBack} style={{ background: "rgba(255,255,255,0.12)", border: "none", color: "#fff", cursor: "pointer", padding: "5px", display: "flex", borderRadius: "6px" }}>
                        <ChevronLeft size={18} />
                    </button>
                )}
                ...
                <div style={{ flex: 1 }}>
                    <div style={{ color: "#fff", fontWeight: 600, fontSize: "14px", lineHeight: 1.2 }}>Mentor Connect</div>
                  
                </div>
                <button onClick={onclose} style={{ background: "rgba(255,255,255,0.12)", border: "none", color: "#fff", cursor: "pointer", padding: "5px", display: "flex", borderRadius: "6px" }}>
                    <ChevronDown size={18} />
                </button>
            </div>
            ...
            {view === "home" && (
                <div style={{ flex: 1, overflowY: "auto", background: "#fff", minHeight: 0 }}>
                    <div style={{ padding: "22px 20px 18px" }}>
                        <h2 style={{ fontSize: "17px", fontWeight: 700, color: "#111", margin: "0 0 4px", letterSpacing: "-0.2px" }}>
                            Hey there, Mentee! 
                        </h2>
                        <p style={{ fontSize: "13px", color: "#777", margin: 0, lineHeight: 1.5 }}>How can we help you today?</p>
                    </div>

                    {/* Get in touch */}
                    <div style={{ padding: "0 16px 6px" }}>
                        <div style={{ fontSize: "11px", fontWeight: 600, color: "#aaa", textTransform: "uppercase", letterSpacing: "0.6px", padding: "0 4px 10px" }}>
                            Get in touch
                        </div>
                        <div style={{ background: "#fff", border: "1px solid rgba(0,152,204,0.18)", borderRadius: "10px", overflow: "hidden" }}>
                            {CONTACT_ITEMS.map((item, i) => (
                                ...
                                >
                                    <div style={iconCircle(item.bg)}>
                                        <span style={{ color: item.color, display: "flex" }}>{getContactIcon(item.key, item.iconSize)}</span>
                                    </div>
                                    <div style={{ flex: 1, minWidth: 0 }}>
                                        <div style={{ fontSize: "13px", fontWeight: 550, color: "#222" }}>{item.label}</div>
                                        <div style={{ fontSize: "11.5px", color: "#999", marginTop: "1px" }}>{item.desc}</div>
                                    </div>
                                    {item.external && <ArrowUpRight size={14} color="#ccc" />}
                                </button>
                            ))}
                        </div>
                    </div>
                    ...
                    <div style={{ padding: "14px 16px 6px" }}>
                        ...
                        <div style={{ background: "#fff", border: "1px solid rgba(0,152,204,0.18)", borderRadius: "10px", overflow: "hidden" }}>
                            {HELP_ITEMS.map((item, i) => (
                                ...
                                >
                                    <div style={iconCircle("#eef8fc")}>
                                        <span style={{ color: "#0098cc", display: "flex" }}>{getHelpIcon(item.key, 16)}</span>
                                    </div>
                                    <div style={{ flex: 1, minWidth: 0 }}>
                                        <div style={{ fontSize: "13px", fontWeight: 550, color: "#222" }}>{item.label}</div>
                                        <div style={{ fontSize: "11.5px", color: "#999", marginTop: "1px" }}>{item.desc}</div>
                    ...
                    <div style={{ padding: "14px 20px 16px", display: "flex", alignItems: "center", gap: "6px" }}>
                        ...
                        <span style={{ fontSize: "11.5px", color: "#bbb" }}>Mon – Sat, 9 AM – 8 PM IST</span>
                    </div>
                </div>
            )}
            ...
            {view === "faq" && (
                <div style={{ flex: 1, overflowY: "auto", background: "#fff", minHeight: 0 }}>
                    <div style={{ padding: "20px 20px 14px" }}>
                        <h3 style={{ fontSize: "16px", fontWeight: 600, color: "#111", margin: "0 0 2px" }}>FAQs</h3>
                        <p style={{ fontSize: "12px", color: "#999", margin: 0 }}>Quick answers to common questions</p>
                    </div>
                        ...
                        <div style={{ background: "#fff", border: "1px solid rgba(0,152,204,0.18)", borderRadius: "10px", overflow: "hidden" }}>
                            {FAQ_DATA.map((faq, i) => (
                                ...
                                >
                                    ...
                                    <span style={{ fontSize: "12.5px", fontWeight: 450, color: "#333", lineHeight: 1.4, flex: 1 }}>{faq.question}</span>
            ...
            {view === "faq-detail" && selectedFaq && (
                <div style={{ flex: 1, overflowY: "auto", background: "#fff", minHeight: 0 }}>
                    <div style={{ padding: "22px 20px" }}>
                        <h3 style={{ fontSize: "14.5px", fontWeight: 600, color: "#111", margin: "0 0 14px", lineHeight: 1.45 }}>{selectedFaq.question}</h3>
                        <p style={{ fontSize: "13px", color: "#555", lineHeight: 1.7, margin: "0 0 24px" }}>{selectedFaq.answer}</p>
                        <div style={{ borderTop: "1px solid rgba(0,152,204,0.1)", paddingTop: "16px" }}>
                            <p style={{ fontSize: "12px", fontWeight: 500, color: "#999", margin: "0 0 10px" }}>Still need help?</p>
                            <div style={{ display: "flex", gap: "8px" }}>
                                <button onClick={openWhatsApp} style={contactBtnStyle("#25D366")} onMouseEnter={hoverBtnIn} onMouseLeave={hoverBtnOut}>
                                    <WhatsAppIcon size={15} /> WhatsApp
                                </button>
                                <button onClick={openCall} style={contactBtnStyle("#0098cc")} onMouseEnter={hoverBtnIn} onMouseLeave={hoverBtnOut}>
                                    <Phone size={15} /> Call
                                </button>
                            </div>
                        </div>
                ...
                <div style={{ flex: 1, display: "flex", flexDirection: "column", background: "#fafbfc", minHeight: 0 }}>
                    ...
                    <div style={{ flex: 1, overflowY: "auto", padding: "16px", minHeight: 0 }}>
                        {chatMessages.map((msg, i) => (
                            <div key={i} style={{ display: "flex", justifyContent: msg.type === "user" ? "flex-end" : "flex-start", marginBottom: "10px", gap: "8px", alignItems: "flex-end" }}>
                                {msg.type === "bot" && (
                                    <div style={{ width: "26px", height: "26px", borderRadius: "50%", background: "#0098cc", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
                                ...
                                <div style={{ maxWidth: "76%" }}>
                                    ...
                                    <div style={{ fontSize: "10px", color: "#bbb", marginTop: "3px", textAlign: msg.type === "user" ? "right" : "left", padding: "0 4px" }}>{msg.time}</div>
                                </div>
                                {msg.type === "user" && (
                                    <div style={{ width: "26px", height: "26px", borderRadius: "50%", background: "rgba(0,152,204,0.1)", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
                        ...
                        {isTyping && (
                            <div style={{ display: "flex", gap: "8px", alignItems: "flex-end", marginBottom: "10px" }}>
                                ...
                                <div style={{ padding: "11px 15px", background: "#fff", border: "1px solid rgba(0,152,204,0.15)", borderRadius: "12px 12px 12px 3px", display: "flex", gap: "4px" }}>
                                    {[0, 1, 2].map((j) => (
                                        <div key={j} style={{ width: "5px", height: "5px", borderRadius: "50%", background: "#bbb", animation: "dotBounce 1.4s infinite ease-in-out", animationDelay: `${j * 0.16}s` }} />
                                    ))}
                                </div>
                            </div>
                        )}
                    ...
                    {/* Quick contact */}
                    <div style={{ padding: "6px 14px", background: "#fff", borderTop: "1px solid rgba(0,152,204,0.1)", display: "flex", gap: "8px", flexShrink: 0 }}>
                        <button onClick={openWhatsApp} style={{ ...contactBtnStyle("#25D366"), fontSize: "11px", padding: "7px 10px", borderRadius: "6px" }} onMouseEnter={hoverBtnIn} onMouseLeave={hoverBtnOut}>
                            <WhatsAppIcon size={13} /> WhatsApp
                        </button>
                        <button onClick={openCall} style={{ ...contactBtnStyle("#0098cc"), fontSize: "11px", padding: "7px 10px", borderRadius: "6px" }} onMouseEnter={hoverBtnIn} onMouseLeave={hoverBtnOut}>
                            <Phone size={13} /> Call Now
                        </button>
                    </div>

                    {/* Input */}
                    <div style={{ padding: "8px 12px 10px", background: "#fff", borderTop: "1px solid rgba(0,152,204,0.1)", display: "flex", gap: "8px", alignItems: "center", flexShrink: 0 }}>
                        <input
                            ref={inputRef}
                            value={inputMsg}
                            ...
                            onBlur={(e) => (e.target.style.borderColor = "rgba(0,152,204,0.2)")}
                        />
                        <button
            ...
            {/* ═══ FOOTER ═══ */}
            {view !== "chat" && (
                <div style={{ padding: "8px 16px", borderTop: "1px solid rgba(0,152,204,0.1)", textAlign: "center", flexShrink: 0 }}>
                    <p style={{ fontSize: "10.5px", color: "#0098cc", margin: 0, fontWeight: 400 }}>Powered by Mentor Connect</p>
                </div>
            )}
        ...
        .mentee-support-widget {
          animation: widgetOpen 0.2s ease-out forwards;
        }
        .mentee-support-widget * { box-sizing: border-box; }
        .mentee-support-widget ::-webkit-scrollbar { width: 3px; }
        .mentee-support-widget ::-webkit-scrollbar-thumb { background: rgba(0,152,204,0.2); border-radius: 3px; }
        .mentee-list-item:hover { background: #f9fafb !important; }
        .mentee-list-item:active { background: #f3f4f6 !important; }
