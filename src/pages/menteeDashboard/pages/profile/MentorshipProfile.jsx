...
const EditProfileModal = ({ isOpen, onClose, section, profileData, onSave, isSaving }) => {
    ...
    useEffect(() => {
        ...
        const isFresher = (profileData?.status || cookieStatus || '').toLowerCase() === 'fresher';
    ...
    const renderExperienceSection = () => {
        ...
        const isFresher = cookieStatus.toLowerCase() === 'fresher';
                    ...
                    {(() => {
                        ...
                        return (
                            ...
                            >
                                {isFresher ? (
                                    <div>
                                        <div className="flex items-center gap-2 px-3 py-2.5 rounded-lg border border-orange-200 bg-orange-50">
                                            <CheckCircle size={14} className="text-orange-500 flex-shrink-0" />
                                            <span className="text-sm font-semibold text-orange-700">0 years</span>
                                        ...
                                        <p className="text-xs text-gray-400 mt-1 flex items-center gap-1">
                                            <AlertCircle size={11} />
                                            Auto-filled as 0 because your profile status is Fresher.
                                        </p>
                                        {/* hidden input keeps the value in formData */}
    ...
    const renderResumeSection = () => (
        <div className="space-y-4">
            <div>
                ...
                <p className="text-xs text-gray-400 mt-0.5">Share your resume with mentors for better guidance</p>
            </div>
            ...
            <div className="rounded-xl border border-orange-100 overflow-hidden">
                ...
                <div className="bg-orange-50 px-4 py-3 grid grid-cols-1 sm:grid-cols-2 gap-2">
                    {[
                        ...
                        <div key={i} className="flex items-start gap-2">
                            <span className="flex-shrink-0 w-4 h-4 rounded-full bg-orange-500 text-white text-[10px] font-bold flex items-center justify-center mt-0.5">
                                {i + 1}
                            </span>
                            <p className="text-xs text-orange-800 leading-relaxed">{step}</p>
                        </div>
                    ))}
                </div>
            </div>
            ...
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
                <p className="text-[11px] text-gray-400 leading-relaxed">
                    Your resume is only shared with mentors you connect with.
                </p>
                {formData.resumeDriveLink && validateDriveLink(formData.resumeDriveLink) && (
                    ...
                    <a href={formData.resumeDriveLink} target="_blank" rel="noopener noreferrer"
                        className="inline-flex items-center gap-1.5 text-xs font-semibold text-orange-600 hover:text-orange-700 bg-orange-50 border border-orange-200 px-3 py-1.5 rounded-lg transition-colors flex-shrink-0"
                    >
...
const MentorshipProfile = () => {
    ...
    const avatarSrc = profile.profilePhotoUrl
        || googleUser?.profile          // ← use Google profile pic
