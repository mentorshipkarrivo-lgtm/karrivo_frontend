...
//     // Update form data whenever profileData changes OR when modal opens
//     useEffect(() => {
//         if (isOpen && profileData) {
//             console.log('Updating form with profileData:', profileData);

//             setFormData({
//                 // About - API data first, then existing formData as fallback
...
//                 yearsOfExperience: profileData?.yearsOfExperience !== undefined ? profileData?.yearsOfExperience : '',
...
//             // Don't close immediately - let parent handle it after refetch
//         } catch (error) {
...
//             }
//             // Don't close modal here - let parent's onSave handle it
//         } catch (error) {
//             console.error("Error saving:", error);
//         }
//     };
...
//             // Refetch will happen automatically due to invalidatesTags
//             // But we call it explicitly to ensure data is fresh
...
//     // ✅ CHANGED: Handle profile data - API first, localStorage as fallback
//     const profile = profileData?.data || {
//         // Only use localStorage values if API data doesn't exist
...
//         // Empty defaults for fields not in localStorage
//         domain: '',
...
const EditProfileModal = ({ isOpen, onClose, section, profileData, onSave, isSaving }) => {
    ...
    // Update form data when modal opens or profileData changes
    useEffect(() => {
        if (isOpen && profileData) {
            console.log('📥 Loading form data from API:', profileData);
            setFormData({
                fullName: profileData.fullName || '',
                email: profileData.email || '',
                phone: profileData.phone || '',
...
const MentorshipProfile = () => {
    ...
    const handleSave = async (updatedData) => {
        try {
            ...
            console.log('🔄 Refetch complete, form will update automatically');
