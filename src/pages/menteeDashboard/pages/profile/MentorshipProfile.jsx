...
//                             storedProfileData.profilePhotoUrl = url;
//                             localStorage.setItem('profileData', JSON.stringify(storedProfileData));
//                         } catch (e) {
//                             console.error('Failed to store photo URL:', e);
//                         }
//                     }
...
//                 setLivePhotoUrl(storedProfileData.profilePhotoUrl);
//             }
//         } catch (e) {
...
//                 try {
//                     const storedProfileData = JSON.parse(localStorage.getItem('profileData') || '{}');
//                     storedProfileData.profilePhotoUrl = updatedData.profilePhotoUrl;
//                     localStorage.setItem('profileData', JSON.stringify(storedProfileData));
//                 } catch (e) {
//                     console.error('Failed to store profile photo in localStorage:', e);
//                 }
//             }
...
const FormField = ({ label, required, children, hint, error }) => (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '4px' }}>
        <label style={{ fontSize: '12px', fontWeight: 700, color: COLORS.primary, letterSpacing: '0.3px', textTransform: 'uppercase' }}>
...
const ProfilePhotoUpload = ({ currentUrl = '', onUpload, userId = 'user' }) => {
    ...
    return (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
            <label style={{ fontSize: '12px', fontWeight: 700, color: COLORS.primary, letterSpacing: '0.3px', textTransform: 'uppercase' }}>Profile Photo</label>
            <div
                onDrop={onDrop}
                onDragOver={e => { e.preventDefault(); setDragging(true); }}
                onDragLeave={() => setDragging(false)}
...
const EditProfileModal = ({ isOpen, onClose, section, profileData, onSave, isSaving, userId, onPhotoUpload }) => {
    ...
    const renderAboutSection = () => (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
            <h3 style={{ margin: 0, fontSize: '15px', fontWeight: 700, color: COLORS.primary, paddingBottom: '12px', borderBottom: `1px solid ${COLORS.border}` }}>Basic Information</h3>
            <ProfilePhotoUpload
                currentUrl={formData.profilePhotoUrl || ''}
                onUpload={async url => {
                    setFormData(prev => ({ ...prev, profilePhotoUrl: url }));
    ...
    const renderExperienceSection = () => {
        ...
        return (
            <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
                <h3 style={{ margin: 0, fontSize: '15px', fontWeight: 700, color: COLORS.primary, paddingBottom: '12px', borderBottom: `1px solid ${COLORS.border}` }}>Professional Experience</h3>
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: '16px' }}>
                    <FormField label="Domain" required error={errors.domain}>
                        <select
                            value={DOMAIN_OPTIONS.includes(formData.domain) || formData.domain === '' ? formData.domain : '__other__'}
    ...
    const renderResumeSection = () => (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
            <div>
                <h3 style={{ margin: '0 0 4px', fontSize: '15px', fontWeight: 700, color: COLORS.primary }}>Resume & Documents</h3>
            ...
            <div style={{ borderRadius: '8px', border: `1px solid ${COLORS.border}`, overflow: 'hidden' }}>
                <div style={{ padding: '10px 16px', display: 'flex', alignItems: 'center', gap: '8px', background: COLORS.primary }}>
                    ...
                    <span style={{ fontSize: '11px', fontWeight: 700, color: COLORS.white, letterSpacing: '0.6px', textTransform: 'uppercase' }}>How to get your Drive link</span>
                </div>
    ...
    const renderGoalsSection = () => (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
            <h3 style={{ margin: 0, fontSize: '15px', fontWeight: 700, color: COLORS.primary, paddingBottom: '12px', borderBottom: `1px solid ${COLORS.border}` }}>Career Goals & Expectations</h3>
                                ...
                                <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
                                    ...
                                    {profile.targetCompanies?.length > 0 && (
                                        <div style={{ paddingTop: '12px', borderTop: `1px solid ${COLORS.mutedBg}` }}>
                                            <p style={{ fontSize: '11px', fontWeight: 700, color: COLORS.muted, textTransform: 'uppercase', letterSpacing: '0.6px', margin: '0 0 8px' }}>Target Companies</p>
                                ...
                                <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
                                    {detailItems.length > 0 && (
                                        <div className="profile-grid" style={{ display: 'grid', gridTemplateColumns: '1fr', gap: '14px' }}>
                                            {detailItems.map((item, i) => (
                                                <div key={i}>
                                                    <p style={{ fontSize: '11px', fontWeight: 700, color: COLORS.muted, textTransform: 'uppercase', letterSpacing: '0.6px', margin: '0 0 4px' }}>{item.label}</p>
                                    ...
                                    {profile.skills?.length > 0 && (
                                        <div>
                                            <p style={{ fontSize: '11px', fontWeight: 700, color: COLORS.muted, textTransform: 'uppercase', letterSpacing: '0.6px', margin: '0 0 8px' }}>Skills</p>
                                            <div style={{ display: 'flex', flexWrap: 'wrap', gap: '6px' }}>
                                                {profile.skills.map((s, i) => <TagPill key={i} label={s} variant="blue" />)}
                                            </div>
                                        </div>
                                    ...
                                    {profile.about && (
                                        <div>
                                            <p style={{ fontSize: '11px', fontWeight: 700, color: COLORS.muted, textTransform: 'uppercase', letterSpacing: '0.6px', margin: '0 0 6px' }}>About</p>
