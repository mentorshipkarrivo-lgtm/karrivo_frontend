...
// //   useEffect(() => {
// //     if (data?.plans) setPlans(data.plans.plans);
// //   }, [data]);
...
// } from "./Mentorpricingapislice"; // add these two exports there
...
// // Map tier_name → subscriber range label
// const TIER_LABELS = {
...
// // ── Derive which tier applies based on subscriber count ───────────────────────
...
// // commission % in tier IS the platform fee (e.g. 12 → platform takes 12%)
// const getPlatformRate = (tier, planKey) => {
//   if (!tier?.commission) return 0.495;
//   const raw = tier.commission[planKey];
//   if (raw == null) return 0.495;
//   return Number(raw) / 100;
// };
// const getMentorRate = (tier, planKey) => 1 - getPlatformRate(tier, planKey);

// // ── Plan Breakdown Modal ───────────────────────────────────────────────────────
// const PlanModal = ({ plan, prices, onClose, activeTier, subscriberCount }) => {
//   const totalMonths = plan.months;
//   const platformRate = getPlatformRate(activeTier, plan.key);
//   const mentorRate = 1 - platformRate;

//   const breakdown = ["experienced", "freshers"].map((tier) => {
//     const totalPrice = Number(prices[tier]) || 0;
//     const perMonth = Math.round(totalPrice / totalMonths);

//     // Platform fee (the tier's commission %)
//     const platformFee = Math.round(totalPrice * platformRate);

//     // CGST 9% + SGST 9% on platform fee
//     const cgst = Math.round(platformFee * CGST_RATE);
//     const sgst = Math.round(platformFee * SGST_RATE);
//     const totalTax = cgst + sgst;

//     // Mentor receives total price minus platform fee minus GST
//     const mentorReceive = totalPrice - platformFee - totalTax;
//     const perMonthGet = Math.round(mentorReceive / totalMonths);

//     return { tier, totalPrice, perMonth, platformFee, cgst, sgst, totalTax, mentorReceive, perMonthGet };
//   });

//   const platformPct = Math.round(platformRate * 100);
//   const mentorPct = Math.round(mentorRate * 100);
...
//     >
//       <div
//         className="w-full max-w-xl bg-[#040f09] border border-[#0c2418] rounded-2xl shadow-2xl flex flex-col"
//         style={{ maxHeight: "92vh" }}
//         onClick={(e) => e.stopPropagation()}
//       >
//         {/* ── Header ── */}
//         <div className="flex items-center justify-between px-4 py-3 border-b border-[#0c2418] shrink-0">
//           <div className="flex items-center gap-3">
//             <div>
//               <h2 className="text-white text-sm font-bold leading-tight">{plan.label}</h2>
//               {activeTier && (
//                 <p className="text-[#4a7a5a] text-[11px] mt-0.5">
//                   Tier: <span className="text-[#0098cc] font-semibold">{TIER_LABELS[activeTier.tier_name] || activeTier.tier_name}</span>
//                   <span className="text-[#2a4a33] mx-1.5">·</span>
//                   Platform <span className="text-white font-semibold">{platformPct}%</span>
//                   <span className="text-[#2a4a33] mx-1.5">·</span>
//                   You keep <span className="text-[#0098cc] font-semibold">{mentorPct}%</span>
//                   <span className="text-[#2a4a33] mx-1.5">·</span>
...
//           <button
//             onClick={onClose}
//             className="w-7 h-7 flex items-center justify-center rounded-lg bg-[#0c2418] hover:bg-[#153a25] text-[#7a9e8a] hover:text-white transition-colors duration-150 cursor-pointer shrink-0"
//           >
...
//             {breakdown.map(({ tier, totalPrice, perMonth, platformFee, cgst, sgst, mentorReceive, perMonthGet }) => (
//               <div key={tier} className="bg-[#020c06] border border-[#0c2418] rounded-xl overflow-hidden flex flex-col">

//                 {/* Column header */}
//                 <div className="px-3 py-2 border-b border-[#0c2418] bg-[#040f09]">
//                   <span className="text-[#0098cc] text-[10px] font-bold uppercase tracking-widest">
//                     {tier === "experienced" ? "Experienced" : "Freshers"}
//                   </span>
//                 </div>
...
//                   {/* Mentee pays */}
//                   <div>
//                     <p className="text-[#3a6a4a] text-[9px] uppercase tracking-widest font-semibold mb-0.5">Mentee Pays</p>
//                     <p className="text-white text-sm font-bold">₹{formatINR(totalPrice)}</p>
//                     {totalMonths > 1 && (
//                       <p className="text-[#3a6a4a] text-[10px]">₹{formatINR(perMonth)}/mo</p>
//                     )}
//                   </div>
...
//                     <p className="text-[#3a6a4a] text-[9px] uppercase tracking-widest font-semibold">Deductions</p>
//                     <div className="flex justify-between items-center">
//                       <span className="text-[#7a9e8a] text-[10px]">Platform ({platformPct}%)</span>
//                       <span className="text-red-400 text-[10px] font-semibold">-₹{formatINR(platformFee)}</span>
//                     </div>
//                     <div className="flex justify-between items-center">
//                       <span className="text-[#4a6a5a] text-[10px]">CGST (9%)</span>
//                       <span className="text-red-400/70 text-[10px]">-₹{formatINR(cgst)}</span>
//                     </div>
//                     <div className="flex justify-between items-center">
//                       <span className="text-[#4a6a5a] text-[10px]">SGST (9%)</span>
//                       <span className="text-red-400/70 text-[10px]">-₹{formatINR(sgst)}</span>
//                     </div>
//                   </div>
...
//                   {/* You receive */}
//                   <div className="bg-[#0098cc]/8 border border-[#0098cc]/15 rounded-lg px-2.5 py-2">
//                     <p className="text-[#3a6a4a] text-[9px] uppercase tracking-widest font-semibold mb-1">You Receive</p>
//                     <p className="text-[#0098cc] text-sm font-bold leading-tight">₹{formatINR(mentorReceive)}</p>
//                     {/* {totalMonths > 1 && (
//                       <p className="text-[#0098cc]/70 text-[10px] font-medium mt-0.5">₹{formatINR(perMonthGet)}</p>
//                     )} */}
//                   </div>
//                 </div>
//               </div>
...
//         {/* ── Footer ── */}
//         <div className="px-4 py-3 border-t border-[#0c2418] shrink-0">
//           <button
//             onClick={onClose}
//             className="w-full bg-[#0098cc] hover:bg-[#007aaa] text-white text-xs font-bold py-2 rounded-xl transition-colors duration-150 cursor-pointer"
//           >
//             Got it
//           </button>
//         </div>
...
// // ── Plan Card ──────────────────────────────────────────────────────────────────
// const PlanCard = ({ plan, prices, onChange, saving, onViewDetails, activeTier }) => {
//   const mentorRate = getMentorRate(activeTier, plan.key);
//   const expMonthly = prices.experienced
//     ? Math.round((Number(prices.experienced) * mentorRate) / plan.months)
//     : 0;
//   const freshMonthly = prices.freshers
//     ? Math.round((Number(prices.freshers) * mentorRate) / plan.months)
//     : 0;
//   const hasData = prices.experienced || prices.freshers;
...
// // ── Commission Tier Badge (shown in header) ────────────────────────────────────
// const TierBadge = ({ activeTier, subscriberCount, tiersLoading }) => {
//   if (tiersLoading) {
//     return (
//       <div className="flex items-center gap-1.5 bg-[#0c2418] px-3 py-1.5 rounded-lg">
//         <div className="w-2 h-2 border border-[#0c2418] border-t-[#0098cc] rounded-full animate-spin" />
//         <span className="text-[#4a7a5a] text-[11px]">Loading tier...</span>
//       </div>
//     );
//   }
...
//   return (
//     <div className="flex items-center gap-2 bg-[#0098cc]/8 border border-[#0098cc]/20 px-3 py-1.5 rounded-lg">
//       <svg className="w-3.5 h-3.5 text-[#0098cc] shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
//   // ── New: subscribers + commission tiers ──────────────────────────────────────
...
//         <p className="mt-6 text-center text-[11px] text-[#2a4a33]">
//           {activeTier
//             ? `Your commission tier: ${TIER_LABELS[activeTier.tier_name] || activeTier.tier_name} · Amounts shown are your estimated monthly earnings`
//             : "Platform fee: 49.5% · Amounts shown are your estimated monthly earnings"}
//         </p>
//       </div>
...
const formatINR = (val) =>
  val != null ? new Intl.NumberFormat("en-IN", { maximumFractionDigits: 0 }).format(val) : "0";

// ── Resolve tier from subscriber count ────────────────────────────────────────
const resolveTier = (subscriberCount, tiers) => {
  if (!tiers || tiers.length === 0) return null;
  const count = Number(subscriberCount) || 0;
  let tierName;
...
// ── Plan Breakdown Modal — reads pre-calculated data from backend ──────────────
      ...
      >
        ...
        <div className="flex items-center justify-between px-4 py-3 border-b border-[#0c2418] shrink-0">
          <div>
            ...
            {tierName && (
              <p className="text-[#4a7a5a] text-[11px] mt-0.5">
                Tier: <span className="text-[#0098cc] font-semibold">{TIER_LABELS[tierName] || tierName}</span>
                <span className="text-[#2a4a33] mx-1.5">·</span>
                Platform <span className="text-white font-semibold">{platformPct}%</span>
                <span className="text-[#2a4a33] mx-1.5">·</span>
                You keep <span className="text-[#0098cc] font-semibold">~{mentorPct}%</span>
          ...
          {!hasBreakdown ? (
            <div className="flex flex-col items-center justify-center py-8 gap-2">
              <p className="text-[#4a7a5a] text-xs text-center">
                No breakdown available yet. Save your pricing to generate a full breakdown.
              </p>
            </div>
          ) : (
            <div className="grid grid-cols-2 gap-3">
              {[
                { label: "Experienced", data: exp },
                { label: "Freshers", data: fre },
              ].map(({ label, data }) => (
                <div
                  key={label}
                  className="bg-[#020c06] border border-[#0c2418] rounded-xl overflow-hidden flex flex-col"
                >
                  {/* Column header */}
                  <div className="px-3 py-2 border-b border-[#0c2418] bg-[#040f09]">
                    <span className="text-[#0098cc] text-[10px] font-bold uppercase tracking-widest">
                  ...
                  <div className="px-3 py-2.5 flex flex-col gap-2 flex-1">
                    {/* Mentee pays */}
                    <div>
                      <p className="text-[#3a6a4a] text-[9px] uppercase tracking-widest font-semibold mb-0.5">
                        Mentee Pays
                      </p>
                      <p className="text-white text-sm font-bold">₹{formatINR(data.totalPrice)}</p>
                      {plan.months > 1 && (
                        <p className="text-[#3a6a4a] text-[10px]">
                          ₹{formatINR(Math.round(data.totalPrice / plan.months))}/mo
                        </p>
                      )}
                    </div>
                    ...
                    {/* Deductions */}
                    <div className="flex flex-col gap-1.5">
                      <p className="text-[#3a6a4a] text-[9px] uppercase tracking-widest font-semibold">
                        Deductions
                      </p>
                      <div className="flex justify-between items-center">
                        <span className="text-[#7a9e8a] text-[10px]">Platform ({data.platformPct}%)</span>
                        <span className="text-red-400 text-[10px] font-semibold">
                          -₹{formatINR(data.platformFee)}
                        </span>
                      </div>
                      <div className="flex justify-between items-center">
                        <span className="text-[#4a6a5a] text-[10px]">CGST (9%)</span>
                        <span className="text-red-400/70 text-[10px]">-₹{formatINR(data.cgst)}</span>
                      </div>
                      <div className="flex justify-between items-center">
                        <span className="text-[#4a6a5a] text-[10px]">SGST (9%)</span>
                        <span className="text-red-400/70 text-[10px]">-₹{formatINR(data.sgst)}</span>
                      </div>
                      <div className="flex justify-between items-center border-t border-[#0c2418] pt-1.5 mt-0.5">
                        <span className="text-[#7a9e8a] text-[10px] font-semibold">Total deducted</span>
                        <span className="text-red-400 text-[10px] font-bold">
                          -₹{formatINR(data.totalDeducted)}
                        </span>
                      </div>
                    </div>
                    ...
                    {/* You receive */}
                    <div className="bg-[#0098cc]/8 border border-[#0098cc]/15 rounded-lg px-2.5 py-2">
                      <p className="text-[#3a6a4a] text-[9px] uppercase tracking-widest font-semibold mb-1">
                        You Receive
                      </p>
                      <p className="text-[#0098cc] text-sm font-bold leading-tight">
                        ₹{formatINR(data.mentorReceive)}
                      </p>
                      {plan.months > 1 && (
                        <p className="text-[#0098cc]/70 text-[10px] font-medium mt-0.5">
                          ₹{formatINR(data.perMonthReceive)}/mo
                        </p>
                      )}
                    </div>
                  </div>
...
const PlanCard = ({ plan, planData, onChange, saving, onViewDetails }) => {
  const prices = {
    experienced: planData?.experienced ?? "",
    freshers: planData?.freshers ?? "",
  };
  const breakdown = planData?.breakdown;
  const hasData = prices.experienced || prices.freshers;
  ...
  return (
    <div className="w-full bg-[#040f09] border border-[#0c2418] rounded-2xl overflow-hidden">
      ...
      {/* Earnings strip — from backend breakdown */}
      {hasBreakdown && (
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mx-5 mb-5 px-4 py-3 bg-[#020c06] border border-[#0c2418] rounded-xl">
          <div className="flex flex-col gap-1">
            <span className="text-[10px] text-[#3a6a4a] uppercase tracking-widest font-semibold">
            ...
            <div className="flex flex-wrap items-center gap-x-3 gap-y-1 mt-0.5">
              <span className="text-xs text-[#7a9e8a]">
                Experienced:&nbsp;
                <strong className="text-[#0098cc] text-sm font-bold">
...
const TierBadge = ({ activeTier, subscriberCount, tiersLoading }) => {
  if (tiersLoading) {
    return (
      <div className="flex items-center gap-1.5 bg-[#0c2418] px-3 py-1.5 rounded-lg w-fit">
        <div className="w-2 h-2 border border-[#0c2418] border-t-[#0098cc] rounded-full animate-spin" />
        <span className="text-[#4a7a5a] text-[11px]">Loading tier...</span>
      </div>
    );
  ...
  return (
    <div className="flex items-center gap-2 bg-[#0098cc]/8 border border-[#0098cc]/20 px-3 py-1.5 rounded-lg w-fit">
      <svg className="w-3.5 h-3.5 text-[#0098cc] shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0z" />
      </svg>
      <span className="text-[#0098cc] text-[11px] font-semibold">
...
const MyPricing = () => {
  ...
  const handleSave = async () => {
    ...
    for (const plan of PLANS) {
      const exp = Number(plans[plan.key].experienced) || 0;
      const fres = Number(plans[plan.key].freshers) || 0;
      if (!exp || !fres) return showToast("error", `Please fill both prices for ${plan.label}`);
      if (fres >= exp) return showToast("error", `${plan.label}: Freshers price must be less than Experienced price`);
    }
    ...
    try {
      ...
      // Backend receives raw prices, calculates breakdown, returns enriched plans
      const result = await saveOrUpdate({ mentorId, plans: sanitizedPlans }).unwrap();

      if (result?.plans?.plans) {
        setFullPlansData(result.plans.plans);
      }
  ...
  return (
    <div className="w-full min-h-screen bg-[#030f0a]">
      <div className="w-full max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="flex flex-col gap-4">
          {PLANS.map((plan) => (
            <PlanCard
              ...
              plan={plan}
              planData={
                fullPlansData?.[plan.key] ?? {
                  experienced: plans[plan.key].experienced,
                  freshers: plans[plan.key].freshers,
                }
              }
              onChange={handleChange}
              saving={saving}
        ...
        <p className="mt-6 text-center text-[11px] text-[#2a4a33]">
          {activeTier
            ? `Commission tier: ${TIER_LABELS[activeTier.tier_name] || activeTier.tier_name} · Breakdown calculated & stored on save`
            : "Platform fee deducted on save · Amounts shown are your estimated earnings"}
        </p>
      </div>
