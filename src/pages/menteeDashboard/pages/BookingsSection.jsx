...
// //       <div className="min-w-0 flex-1">
// //         <p className="text-[11px] text-slate-400 mb-0.5">{label}</p>
// //         {children || <p className="text-sm font-medium text-slate-700 break-words">{value || "—"}</p>}
// //       </div>
// //     </div>
// //   );
// // }
...
// //     >
// //       {/* LEFT: Image Section */}
// //       <div className="relative w-[140px] sm:w-[180px] h-full bg-gradient-to-br from-slate-100 to-slate-200 flex-shrink-0 overflow-hidden">
// //         {hasImage ? (
// //           <>
// //             <img
// //               src={mentor.profilePhoto || mentor.profileImage}
...
// //             />
// //             <div className="hidden w-full h-full bg-gradient-to-br from-blue-50 to-blue-100 items-center justify-center">
// //               <span className="text-3xl sm:text-4xl font-bold text-blue-400">{initials}</span>
// //             </div>
// //           </>
// //         ) : (
// //           <div className="w-full h-full bg-gradient-to-br from-blue-50 to-blue-100 flex items-center justify-center">
...
// //         {/* Rating Badge */}
// //         <div className="absolute top-2 right-2 bg-white shadow-md rounded-full px-1.5 py-0.5 flex items-center gap-0.5 border border-slate-100">
...
// //           </p>
// //           {mentor.companyName && (
// //             <p className="flex items-center gap-1 text-[11px] sm:text-xs text-slate-400 mt-0.5 truncate">
...
// //           <div className="flex flex-wrap gap-1 mb-2">
// //             {mentor.guidanceAreas.slice(0, 2).map((area, i) => (
// //               <span key={i} className="inline-block text-[8px] bg-blue-50 text-blue-600 px-1.5 py-0.5 rounded-sm truncate max-w-[80px]">
// //                 {area}
// //               </span>
// //             ))}
// //             {mentor.guidanceAreas.length > 2 && (
...
// //         <button
// //           onClick={() => onViewProfile(mentor)}
// //           className="w-full py-1.5 rounded-lg font-semibold text-[11px] sm:text-xs text-white
// //                      transition-all duration-150 active:scale-[0.97] shadow-sm"
// //           style={{ backgroundColor: COLOR.dark }}
// //           onMouseEnter={(e) => e.target.style.backgroundColor = COLOR.primary}
// //           onMouseLeave={(e) => e.target.style.backgroundColor = COLOR.dark}
...
// // // ═══════════════════════════════════════════════════════════════════════════
// // // RESCHEDULE MODAL
// // // KEY FIX: z-index is z-[300] (above session panel z-50), full-screen on mobile,
// // // centered modal on desktop so it never overlaps/touches the side panel
// // // ═══════════════════════════════════════════════════════════════════════════
...
// //   return (
// //     <>
// //       {/* Backdrop — z-[290] so it covers the side panel (z-50) but is below the modal */}
// //       <div
// //         onClick={onClose}
...
// //       {/* Modal container — z-[300], centered on all screens */}
// //       <div
// //         className={`fixed inset-0 z-[300] flex items-end sm:items-center justify-center p-0 sm:p-4
// //           transition-all duration-200 pointer-events-none`}
// //       >
// //         <div
// //           className={`bg-white w-full sm:rounded-2xl rounded-t-2xl sm:max-w-2xl max-h-[92vh] sm:max-h-[85vh] flex flex-col
// //             border border-[#e2e8f0] font-[DM_Sans,sans-serif] pointer-events-auto
// //             transition-all duration-250 ease-out
// //             ${isOpen ? "opacity-100 translate-y-0 scale-100" : "opacity-0 translate-y-8 scale-95"}`}
// //         >
// //           {/* Header */}
// //           <div className="flex items-center gap-3 px-4 sm:px-5 py-3.5 sm:py-4 border-b border-[#e2e8f0] flex-shrink-0">
// //             <button
// //               onClick={onClose}
// //               className="flex items-center gap-1.5 text-sm font-medium text-[#64748b] hover:text-[#1a1a2e] transition-colors"
// //             >
...
// //             {/* Current session info */}
// //             <div className="mb-4 sm:mb-5 p-3 sm:p-3.5 rounded-xl bg-white border border-[#e2e8f0]">
// //               <p className="text-xs text-[#94a3b8] mb-1">Current Session</p>
// //               <p className="text-sm font-semibold text-[#1a1a2e]">{getMentorName(booking)}</p>
// //               <p className="text-xs text-[#64748b] mt-0.5">
// //                 {fmtDate(booking.sessionDate)} at {formatCardTime(booking.startTime)}
...
// //                           key={slot._id || idx}
// //                           onClick={() => { setSelectedSlot(slot); setStep("confirm"); }}
// //                           className="text-left p-2.5 rounded-xl border border-[#e2e8f0] hover:border-[#008dbe] hover:bg-[#008dbe0a] transition-all duration-150 w-full"
// //                         >
// //                           <div className="flex flex-col items-center gap-1.5 text-center">
// //                             <div className="w-full rounded-lg bg-[#e6f7fc] flex flex-col items-center justify-center py-1.5">
// //                               <span className="text-[9px] font-bold uppercase text-[#008dbe]">{dp.month}</span>
// //                               <span className="text-lg font-bold text-[#1a1a2e] leading-none">{dp.day}</span>
// //                             </div>
// //                             <div>
...
// //                 <p className="text-sm font-medium text-[#1a1a2e] mb-4">Confirm Reschedule</p>
// //                 <div className="grid grid-cols-2 gap-3 mb-4">
// //                   <div className="p-3 sm:p-3.5 rounded-xl bg-[#f8fafc] border border-[#e2e8f0]">
// //                     <p className="text-[10px] text-[#94a3b8] mb-1.5 uppercase font-semibold tracking-wide">Current</p>
// //                     <p className="text-sm font-semibold text-[#1a1a2e]">{formatCardDate(booking.sessionDate)}</p>
// //                     <p className="text-xs text-[#64748b] mt-0.5">{formatCardTime(booking.startTime)}</p>
// //                   </div>
// //                   <div className="p-3 sm:p-3.5 rounded-xl bg-[#e6f7fc] border border-[#008dbe]/30">
// //                     <p className="text-[10px] text-[#008dbe] mb-1.5 uppercase font-semibold tracking-wide">New Slot</p>
// //                     <p className="text-sm font-semibold text-[#1a1a2e]">{formatSlotDate(selectedSlot.date)}</p>
// //                     <p className="text-xs text-[#008dbe] mt-0.5">
// //                       {to12h(selectedSlot.startTime)} — {to12h(selectedSlot.endTime)}
...
// //           {/* Footer */}
// //           {step === "confirm" && selectedSlot && (
// //             <div className="border-t border-[#e2e8f0] px-4 sm:px-5 py-3.5 sm:py-4 flex items-center justify-between bg-white rounded-b-2xl flex-shrink-0">
// //               <button
// //                 onClick={() => setStep("slots")}
// //                 className="text-sm text-[#64748b] hover:text-[#1a1a2e] transition-colors flex items-center gap-1.5"
// //               >
...
// //   const mentorName = getMentorName(booking);
// //   const mentorImage = typeof booking.mentorId === "object"
// //     ? (booking.mentorId?.profilePhoto || booking.mentorId?.profileImage)
// //     : null;
...
// //       transition={{ duration: 0.25 }}
// //       className={`group relative flex items-stretch border-b border-slate-100 last:border-b-0
// //         ${isDisabled ? "opacity-60 grayscale pointer-events-none cursor-not-allowed" : "hover:bg-slate-50/50"}
// //         ${isCancelled ? "border-l-4 border-red-400" : ""}
// //         ${booking.isExpired ? "border-l-4 border-orange-400" : ""}
// //         transition-colors duration-150 font-[DM_Sans,sans-serif] overflow-hidden bg-white`}
// //     >
// //       {/* Date column */}
// //       <div className="flex-shrink-0 w-12 sm:w-16 flex flex-col items-center justify-center py-3 sm:py-4 border-r border-slate-100 bg-white">
// //         <span className="text-[8px] sm:text-[9px] font-bold uppercase tracking-widest" style={{ color: COLOR.primary }}>
// //           {MONTH_SHORT[new Date(booking.sessionDate).getMonth()]}
// //         </span>
// //         <span className="text-lg sm:text-2xl font-bold text-slate-900 leading-tight">
// //           {new Date(booking.sessionDate).getDate()}
// //         </span>
...
// //       {/* Main content */}
// //       <div className="flex-1 min-w-0 px-2.5 sm:px-4 py-2.5 sm:py-3 bg-white">
// //         <p className="text-[9px] sm:text-[10px] font-semibold uppercase tracking-wide mb-0.5" style={{ color: COLOR.primary }}>
// //           {booking.sessionType || "1:1 Session"}
// //         </p>

// //         <h3 className="text-xs sm:text-sm font-semibold text-slate-900 truncate">{booking.mentorName}</h3>
// //         <p className="text-[11px] sm:text-xs text-slate-400 mt-0.5 truncate">
// //           {booking.topic || getMentorSubtitle(booking) || "Mentorship Session"}
// //         </p>

// //         <div className="flex flex-wrap items-center gap-1 sm:gap-1.5 mt-1 sm:mt-1.5">
// //           <span className="inline-flex items-center gap-0.5 sm:gap-1 text-[9px] sm:text-[10px] text-slate-400">
...
// //           {booking.isFreeSession && (
// //             <span className="inline-flex items-center gap-0.5 text-[9px] font-bold bg-emerald-50 text-emerald-600 border border-emerald-200 rounded-full px-1.5 py-0.5">
// //               <Zap className="w-2 h-2 sm:w-2.5 sm:h-2.5" /> FREE
// //             </span>
// //           )}

// //           {booking.isExpired && (
// //             <span className="inline-flex items-center text-[9px] font-bold bg-red-50 text-red-600 border border-red-200 rounded-full px-1.5 py-0.5">
// //               EXPIRED
// //             </span>
// //           )}
// //         </div>
...
// //             onClick={() => !booking.isExpired && onOpenPanel(booking, "details")}
// //             disabled={booking.isExpired}
// //             className={`flex items-center gap-1 px-2 sm:px-2.5 py-1 sm:py-1.5 rounded-md text-[10px] sm:text-[11px] font-semibold
// //               text-white transition-all active:scale-[0.97]
// //               ${booking.isExpired ? "bg-gray-400 cursor-not-allowed opacity-60" : "hover:opacity-90"}`}
// //             style={{ backgroundColor: booking.isExpired ? "#94a3b8" : COLOR.primary }}
// //           >
// //             <Eye className="w-2.5 h-2.5 sm:w-3 sm:h-3" /> View Details
// //           </button>

// //           {booking.paymentStatus === "unpaid" && (
// //             <button
// //               onClick={() => onCompletePayment(booking)}
// //               className="flex items-center gap-1 px-2 sm:px-3 py-1 sm:py-1.5 rounded-md text-[10px] sm:text-[11px] font-semibold
// //                 text-white bg-[#1a1a2e] active:scale-[0.97] transition-all"
// //             >
// //               <CreditCard className="w-2.5 h-2.5 sm:w-3 sm:h-3" /> Complete Payment
// //             </button>
// //           )}

// //           {isCancelled && (
// //             <span style={{ color: '#dc2626' }} className="inline-flex items-center px-2 sm:px-3 py-1 sm:py-1.5 rounded-full text-[9px] sm:text-[10px] font-bold tracking-widest uppercase bg-red-50 border border-red-200">
// //               Session Cancelled
// //             </span>
// //           )}
...
// //               <button
// //                 disabled
// //                 className="flex items-center gap-1 px-2 sm:px-2.5 py-1 sm:py-1.5 rounded-md text-[10px] sm:text-[11px] font-semibold border cursor-not-allowed opacity-70"
// //                 style={{ color: '#ef4444', borderColor: '#fecaca', backgroundColor: '#fef2f2' }}
// //               >
// //                 <Video className="w-2.5 h-2.5 sm:w-3 sm:h-3" /> Expired
// //               </button>
// //             ) : (
// //               <button
// //                 disabled={!canJoin}
// //                 onClick={() => window.open(booking.meetingLink, "_blank")}
// //                 className="flex items-center gap-1 px-2 sm:px-2.5 py-1 sm:py-1.5 rounded-md text-[10px] sm:text-[11px] font-semibold border"
// //                 style={{
// //                   color: canJoin ? COLOR.primary : '#94a3b8',
// //                   borderColor: canJoin ? COLOR.primary : COLOR.border,
// //                   backgroundColor: canJoin ? 'white' : '#f1f5f9',
...
// //       {/* Mentor image — hidden on xs, visible sm+ */}
// //       <div className="flex-shrink-0 w-[64px] sm:w-[90px] md:w-[100px] self-stretch relative overflow-hidden hidden sm:block">
// //         {mentorImage ? (
// //           <img src={mentorImage} alt={mentorName} className="w-full h-full object-cover object-top" />
// //         ) : (
// //           <div className="w-full h-full bg-gradient-to-br from-slate-700 via-slate-800 to-blue-900 flex items-center justify-center">
// //             <span className="text-base sm:text-xl font-bold text-white/80 select-none">
// //               {getMentorInitials(booking)}
// //             </span>
// //           </div>
// //         )}
...
// // // MAIN DASHBOARD
// // // KEY FIXES:
// // // 1. Side panel z-50, Reschedule modal z-[300] — they never conflict
// // // 2. When reschedule opens, side panel stays behind the backdrop
// // // 3. Side panel is full-width on mobile, 400px on sm+
// // // 4. Proper body scroll lock per active modal
// // // ═══════════════════════════════════════════════════════════════════════════
...
// //   // Body scroll lock — only lock when the topmost modal is open
// //   useEffect(() => {
...
// //   // Handlers
// //   const openPanel = (booking, view = "details") => {
// //     // If requesting reschedule from inside panel, close panel first then open reschedule
// //     if (view === "reschedule") {
// //       setPanelOpen(false);
// //       setTimeout(() => {
...
// //                 Failed to load mentors. Please refresh.
// //               </div>
// //             )}
// //             {!mentorsLoading && !mentorsError && filtered.length === 0 && (
// //               <div className="text-center py-10 text-sm text-slate-400">No mentors found.</div>
...
// //             {/* Status badges */}
// //             {bookings.length > 0 && (
// //               <div className="flex items-center gap-2 mt-4 sm:mt-5 mb-3 sm:mb-4 flex-wrap">
// //                 {statusCounts.unattended > 0 && (
// //                   <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-semibold bg-orange-50 text-orange-600 border border-orange-200">
// //                     Unattended
// //                     <span className="bg-orange-200 text-orange-700 w-4 h-4 rounded-full flex items-center justify-center text-[10px] font-bold">
...
// //             {!bookingsLoading && !bookingsError &&
// //               bookings.filter(b => b.paymentStatus === "Approved" || b.paymentStatus === "pending").length === 0 && (
// //                 <div className="border border-slate-200 rounded-xl p-4 mb-4 sm:mb-5 bg-white mt-3 sm:mt-4">
// //                   <p className="text-sm font-semibold text-slate-900">No Upcoming Trial Bookings</p>
// //                   <p className="text-sm text-slate-500 mt-0.5">Book one from mentors recommended for you</p>
// //                   <p className="text-xs text-slate-400 mt-1">
// //                     Trials are 100% free and a great opportunity to finalise the mentor who's meant for you.
...
// //       {/* ─── RESCHEDULE MODAL (z-[300]) — always above side panel ─── */}
// //       <RescheduleModal
// //         booking={rescheduleTarget}
// //         isOpen={rescheduleOpen}
// //         onClose={closeReschedule}
...
// //       {/* ─── SESSION DETAILS SIDE PANEL (z-50) ─── */}
...
// //       {/* Side panel — full width on mobile, 400px on sm+ */}
// //       <div className={`
// //         fixed top-0 right-0 h-full z-50 bg-white
...
// //             {panelView === "details" && (
// //               <>
// //                 <div className="flex items-center gap-3 px-4 sm:px-5 py-3.5 sm:py-4 border-b border-slate-100 flex-shrink-0">
// //                   <button
// //                     onClick={closePanel}
// //                     className="text-slate-400 transition-colors duration-150 hover:text-blue-600"
// //                   >
...
// //                           target="_blank"
// //                           rel="noopener noreferrer"
// //                           className="text-sm font-medium flex items-center gap-1 transition-colors duration-150 hover:opacity-70 break-all"
// //                           style={{ color: COLOR.primary }}
// //                         >
...
// //                 <div className="flex-shrink-0 px-4 sm:px-5 py-3.5 sm:py-4 border-t border-slate-100 space-y-2">
// //                   {selectedBooking.paymentStatus === "unpaid" && (
// //                     <button
// //                       onClick={() => handleCompletePayment(selectedBooking)}
// //                       className="flex items-center justify-center gap-2 w-full py-2.5 rounded-lg font-semibold text-sm
// //                         text-white bg-[#1a1a2e] transition-all hover:opacity-90"
// //                     >
// //                       <CreditCard className="w-4 h-4" /> Complete Payment
// //                     </button>
// //                   )}
...
// //                       onClick={() => setPanelView("cancel")}
// //                       className="w-full py-2.5 rounded-lg font-semibold text-sm transition-colors duration-150 border hover:opacity-90"
// //                       style={{ backgroundColor: "white", color: COLOR.dark, borderColor: COLOR.border }}
// //                     >
// //                       Cancel Booking
// //                     </button>
// //                   )}
...
// //                   <div className="p-3 sm:p-3.5 rounded-xl bg-slate-50 border border-slate-200 mb-4">
// //                     <p className="text-sm font-semibold text-slate-800">{getMentorName(selectedBooking)}</p>
// //                     <p className="text-xs text-slate-500 mt-1">
// //                       {formatCardDate(selectedBooking.sessionDate)} · {formatCardTime(selectedBooking.startTime)} · {selectedBooking.durationMinutes} min
// //                     </p>
...
// //                 <div className="flex-shrink-0 px-4 sm:px-5 py-3.5 sm:py-4 border-t border-slate-100 flex gap-3">
// //                   <button
// //                     onClick={() => { setPanelView("details"); setCancelReason(""); }}
// //                     className="flex-1 py-2.5 rounded-lg font-medium text-sm transition-colors duration-150 border hover:opacity-90"
// //                     style={{ backgroundColor: "white", color: COLOR.dark, borderColor: COLOR.border }}
...
//       {/* ─── SESSION DETAILS SIDE PANEL (z-[70]) — above sidebar (z-50) & header (z-40) ─── */}
//       {/* Backdrop for side panel */}
//       <div
//         onClick={closePanel}
//         className={`fixed inset-0 z-[60] transition-all duration-200 bg-black/20
//           ${panelOpen ? "opacity-100 pointer-events-auto" : "opacity-0 pointer-events-none"}`}
//       />

//       {/* Side panel: starts BELOW the header on sm+ so sidebar nav is never blocked.
//           top-[89px] = announcement bar (~33px) + header (~56px).
//           On mobile it goes full height (has its own X close button). */}
//       <div className={`
//         fixed top-0 sm:top-[89px] right-0
//         h-full sm:h-[calc(100vh-89px)]
//         z-[70] bg-white
//         w-full sm:w-[380px] md:w-[400px]
//         transform transition-transform duration-250 ease-out
//         flex flex-col border-l border-slate-200 shadow-xl
//         font-[DM_Sans,sans-serif]
//         ${panelOpen ? "translate-x-0" : "translate-x-full"}
//       `}>
//         {selectedBooking && (
...
//   // Add this inside BookingsDashboard, before the return statement
...
//   const start = getSessionStartDateTime(booking);
//   const end = new Date(start.getTime() + (booking.durationMinutes || 30) * 60000);
//   const joinableFrom = new Date(start.getTime() + 10 * 60 * 1000); // enable AFTER 10 min
//   const canJoin = now >= joinableFrom && now <= end;
...
//               <button
//                 disabled={!canJoin}
//                 onClick={() => canJoin && window.open(booking.meetingLink, "_blank")}
//                 title={!canJoin ? "Join button enables 10 minutes after session starts" : "Join Meeting"}
//                 className="flex items-center gap-1 px-2 sm:px-2.5 py-1 sm:py-1.5 rounded-md text-[10px] sm:text-[11px] font-semibold border transition-all"
//                 style={{
//                   color: canJoin ? COLOR.primary : '#94a3b8',
//                   borderColor: canJoin ? COLOR.primary : COLOR.border,
//                   backgroundColor: canJoin ? 'white' : '#f1f5f9',
...
//       <div
//         onClick={closePanel}
//         className={`fixed inset-0 sm:top-[89px] z-[60] transition-all duration-200 bg-black/20
//           ${panelOpen ? "opacity-100 pointer-events-auto" : "opacity-0 pointer-events-none"}`}
//       />
...
//           On mobile it goes full height (has its own X close button). */}
//       <div className={`
//         fixed top-0 sm:top-[89px] bottom-0 right-0
//         z-[70] bg-white
//         w-full sm:w-[380px] md:w-[400px]
//         transform transition-transform duration-250 ease-out
//         flex flex-col border-l border-slate-200 shadow-xl
...
//                   {selectedBooking.paymentStatus === "Approved" && selectedBooking.meetingLink && (() => {
//                     const panelStart = getSessionStartDateTime(selectedBooking); // won't work here — use inline calc
//                     const now2 = new Date();
//                     const pStart = (() => {
//                       const date = new Date(selectedBooking.sessionDate);
//                       const time = (selectedBooking.startTime || "").toLowerCase().trim();
//                       let hours, minutes;
//                       if (time.includes("am") || time.includes("pm")) {
//                         const clean = time.replace(/am|pm/g, "").trim();
//                         [hours, minutes] = clean.split(":").map(Number);
...
//                       return date;
//                     })();
//                     const pEnd = new Date(pStart.getTime() + (selectedBooking.durationMinutes || 30) * 60000);
//                     const pCanJoin = now2 >= new Date(pStart.getTime() + 10 * 60 * 1000) && now2 <= pEnd;

//                     return pCanJoin ? (

//                       <a href={selectedBooking.meetingLink}
//                         target="_blank"
//                         rel="noopener noreferrer"
//                         className="flex items-center justify-center gap-2 w-full py-2.5 rounded-lg font-semibold text-sm text-white transition-colors duration-150 hover:opacity-90"
//                         style={{ backgroundColor: COLOR.dark }}
...
//                       <button
//                         disabled
//                         title="Join enables 10 minutes after session starts"
//                         className="flex items-center justify-center gap-2 w-full py-2.5 rounded-lg font-semibold text-sm cursor-not-allowed opacity-50"
...
// ═══════════════════════════════════════════════════════════════════════════
// SHARED TIME PARSER — used by both BookingCard and BookingsDashboard panel
// ═══════════════════════════════════════════════════════════════════════════
const parseSessionStart = (booking) => {
  const date = new Date(booking.sessionDate);
  const time = (booking.startTime || "").toLowerCase().trim();
...
function BookingCard({
  ...
  // Use shared parseSessionStart helper
  const start = parseSessionStart(booking);
  const end = new Date(start.getTime() + (booking.durationMinutes || 30) * 60000);
  const joinableFrom = new Date(start.getTime() + 10 * 60 * 1000); // enable AFTER 10 min from start
  const canJoin = now >= joinableFrom && now <= end;
...
export default function BookingsDashboard() {
  ...
  // Panel join button logic — uses shared parseSessionStart
  ...
  return (
    <main className="bg-white font-[DM_Sans,sans-serif] min-h-screen">
      ...
      {/* ─── SESSION DETAILS SIDE PANEL (z-[70]) ─── */}
      <div className={`
        fixed top-0 sm:top-[89px] bottom-0 right-0
        z-[70] bg-white
        w-full sm:w-[380px] md:w-[400px]
            ...
            {panelView === "details" && (
              <>
                ...
                <div className="flex-shrink-0 px-4 sm:px-5 py-3.5 sm:py-4 border-t border-slate-100 space-y-2">
                  ...
                  {/* Join Meeting button in panel — uses getPanelCanJoin */}
                  {selectedBooking.paymentStatus === "Approved" && selectedBooking.meetingLink && (
