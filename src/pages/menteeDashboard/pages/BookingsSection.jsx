...
export default function BookingsDashboard() {
  ...
  return (
    /*
      ✅ KEY RESPONSIVE FIX:
      - Mobile/tablet  → normal page scroll (overflow auto, no fixed height)
      - Desktop (lg+)  → two-column layout, left col scrolls, right sidebar sticky
      No more h-[85vh] overflow-hidden trapping both columns in a split-scroll
    */
    <main className="bg-white font-[DM_Sans,sans-serif] min-h-screen">
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 py-5 sm:py-6">
        ...
        {/* ── TWO-COLUMN WRAPPER (desktop only splits; mobile stacks) ── */}
        <div className="flex flex-col lg:flex-row gap-6">

          {/* ── LEFT COLUMN — natural height on mobile, scroll on desktop ── */}
          <div className="flex-1 min-w-0">
            ...
            {/* Mentor card row — horizontal scroll on mobile, wrap on desktop */}
            {!mentorsLoading && !mentorsError && filtered.length > 0 && (
              <div className="
                flex gap-3 pb-3
                overflow-x-auto sm:overflow-x-auto lg:overflow-x-visible
                lg:flex-wrap
                snap-x snap-mandatory
                scrollbar-thin scrollbar-color: slate-200
          ...
          {/* ── RIGHT SIDEBAR — desktop only (mobile already shown above) ── */}
      ...
      {/* ══ SLIDE PANEL — full-width on mobile, 400px on sm+ ═══════════════ */}
      <div className={`
        ...
        {selectedBooking && (
          <>
            {panelView === "details" && (
              <>
                ...
                <div className="flex-shrink-0 px-5 py-4 border-t border-slate-100 space-y-2">
                  {selectedBooking.paymentStatus === "Approved" && selectedBooking.meetingLink && (
                    <a href={selectedBooking.meetingLink} target="_blank" rel="noopener noreferrer"
                      className="flex items-center justify-center gap-2 w-full py-2.5 rounded-lg font-semibold text-sm text-white bg-blue-600 hover:bg-blue-700">
                      <Video className="w-4 h-4" /> Join Meeting
                    </a>
                  )}
                </div>
            ...
            {panelView === "cancel" && (
              <>
                <div className="flex items-center gap-3 px-5 py-4 border-b border-slate-100 flex-shrink-0">
                  <button onClick={() => setPanelView("details")} className="text-slate-400 hover:text-slate-600">
                    <ArrowLeft className="w-5 h-5" />
                  </button>
                  <h2 className="text-sm font-semibold text-slate-900">Cancel Booking</h2>
                ...
                <div className="flex-1 overflow-y-auto px-5 py-5">
                  ...
                  <textarea
                    ...
                    placeholder="Please let us know why you're cancelling…" rows={5}
                ...
                <div className="flex-shrink-0 px-5 py-4 border-t border-slate-100 flex gap-3">
                  <button onClick={() => { setPanelView("details"); setCancelReason(""); }}
                    className="flex-1 border border-slate-200 py-2.5 rounded-lg font-medium text-sm text-slate-600 hover:bg-slate-50 active:scale-[0.97]">
                    Keep Booking
                  </button>
                  <button onClick={handleCancelBooking} disabled={isCancelling}
                    className="flex-1 bg-red-500 hover:bg-red-600 text-white py-2.5 rounded-lg font-semibold text-sm disabled:opacity-50 disabled:cursor-not-allowed active:scale-[0.97]">
                    {isCancelling
                      ? <span className="flex items-center justify-center gap-2"><Loader2 className="w-4 h-4 animate-spin" /> Cancelling…</span>
                      : "Yes, Cancel"
                    }
