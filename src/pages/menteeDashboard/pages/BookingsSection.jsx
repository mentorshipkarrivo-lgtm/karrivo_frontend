...
// // ═══════════════════════════════════════════════════════════════════════════
// // MENTOR CARD — fully adaptive (vertical on narrow, horizontal on wide)
// // ═══════════════════════════════════════════════════════════════════════════
// function MentorCard({ mentor, index, onViewProfile }) {
//   const initials = getInitials(mentor.fullName);
//   const hasImage = mentor.profilePhoto || mentor.profileImage;
...
//                  font-[DM_Sans,sans-serif] flex flex-col"
//     >
//       {/* Photo — fixed height, full width, never cropped away */}
//       <div className="relative w-full bg-gradient-to-br from-slate-100 to-slate-200 flex-shrink-0"
//         style={{ height: '140px' }}>
//         {hasImage ? (
//           <>
//             <img
...
//               }}
//             />
//             {/* Fallback initials (hidden until img errors) */}
...
//         {/* Rating badge */}
//         <div className="absolute top-2 right-2 bg-white shadow-sm rounded-full px-2 py-0.5
//                         flex items-center gap-0.5 border border-slate-100">
//           <Star className="w-3 h-3 fill-yellow-400 text-yellow-400" />
//           <span className="text-[10px] font-bold text-slate-800">
//             {mentor.rating ? Number(mentor.rating).toFixed(1) : "5.0"}
//           </span>
...
//         {/* Company */}
//         {mentor.companyName && (
//           <p className="flex items-center gap-1 text-[11px] text-slate-400 mt-0.5 line-clamp-1">
//             <Briefcase className="w-3 h-3 flex-shrink-0" />
//             <span className="truncate">{mentor.companyName}</span>
//           </p>
//         )}
...
//             {mentor.guidanceAreas.slice(0, 2).map((area, i) => (
//               <span key={i}
//                 className="inline-block text-[9px] bg-blue-50 text-blue-600 px-1.5 py-0.5 rounded-sm leading-tight max-w-full truncate"
//                 style={{ maxWidth: '100%' }}>
//                 {area}
//               </span>
//             ))}
...
//         <button
//           onClick={() => onViewProfile(mentor)}
//           className="mt-3 w-full py-2 rounded-lg font-semibold text-xs text-white
//                      transition-all duration-150 active:scale-[0.97] shadow-sm"
//           style={{ backgroundColor: COLOR.dark }}
//           onMouseEnter={(e) => (e.currentTarget.style.backgroundColor = COLOR.primary)}
//           onMouseLeave={(e) => (e.currentTarget.style.backgroundColor = COLOR.dark)}
//         >
//           View Profile
//         </button>
//       </div>
...
//       <div
//         onClick={onClose}
//         className={`fixed inset-0 z-50 transition-all duration-300 bg-black/30 backdrop-blur-md
...
//           {/* Header */}
//           <div className="flex items-center gap-3 px-5 py-4 border-b border-[#e2e8f0] flex-shrink-0">
//             <button onClick={onClose} className="flex items-center gap-1.5 text-sm font-medium text-[#64748b] hover:text-[#1a1a2e] transition-colors">
//               <ArrowLeft className="w-4 h-4" /> Back
//             </button>
//             <div className="flex-1 text-center">
//               <h2 className="text-sm font-semibold text-[#1a1a2e]">Reschedule Session</h2>
...
//                 )}
//                 {slotsError && (
//                   <div className="text-center py-10 text-sm text-[#94a3b8]">Could not load slots.</div>
//                 )}
//                 {!slotsLoading && !slotsError && availableSlots.length === 0 && (
//                   <div className="text-center py-10 text-sm text-[#94a3b8]">No available slots right now.</div>
//                 )}
//                 {!slotsLoading && !slotsError && availableSlots.length > 0 && (
...
//                         >
//                           <div className="flex flex-col items-center gap-1.5 text-center">
//                             <div className="rounded-lg bg-[#e6f7fc] flex flex-col items-center justify-center w-full py-1.5">
//                               <span className="text-[9px] font-bold uppercase text-[#008dbe]">{dp.month}</span>
//                               <span className="text-lg font-bold text-[#1a1a2e] leading-none">{dp.day}</span>
//                             </div>
//                             <div>
//                               <p className="text-xs font-semibold text-[#1a1a2e]">{getSlotDayName(slot.date)}</p>
//                               <p className="text-[10px] text-[#64748b]">{to12h(slot.startTime)}</p>
//                               <p className="text-[10px] text-[#64748b]">— {to12h(slot.endTime)}</p>
//                               <p className="text-[10px] text-[#94a3b8] mt-0.5">{mins} min</p>
//                             </div>
//                           </div>
//                         </button>
//                       );
...
//     >
//       {/* Date column */}
//       <div className="flex-shrink-0 w-12 sm:w-16 flex flex-col items-center justify-center py-4 border-r border-slate-100 bg-white">
...
//       {/* Mentor thumbnail — hidden on xs, visible sm+ */}
//       <div className="flex-shrink-0 w-[70px] sm:w-[90px] self-stretch relative overflow-hidden hidden xs:flex">
//         {mentorImage ? (
//           <img src={mentorImage} alt={mentorName} className="w-full h-full object-cover object-top" />
//         ) : (
//           <div className="w-full h-full bg-gradient-to-br from-slate-700 via-slate-800 to-blue-900 flex items-center justify-center">
...
//             {/*
//               Mentor card grid:
//               - 1 col  on mobile (< 480px)
//               - 2 cols on sm+ (480px+)
//               - 3 cols on md+ (768px+) when sidebar is gone
//               - 4 cols on xl+ if space allows
//               Cards are purely vertical (photo top, content bottom, button always at bottom).
//             */}
//             {!mentorsLoading && !mentorsError && filtered.length > 0 && (
//               <div
...
//           {/* <div className="hidden lg:block lg:w-[260px] xl:w-[280px] flex-shrink-0 lg:sticky lg:top-6 h-fit">
//             <SidebarWidgets navigate={navigate} />
//           </div> */}
//         </div>
//       </div>
...
//               <>
//                 <div className="flex items-center gap-3 px-5 py-4 border-b border-slate-100 flex-shrink-0">
//                   <button onClick={closePanel} className="text-slate-400 hover:text-blue-600 transition-colors">
//                     <X className="w-5 h-5" />
//                   </button>
//                   <h2 className="text-sm font-semibold text-slate-900">Session Details</h2>
//                 </div>
...
//                           className="text-sm font-medium flex items-center gap-1 hover:opacity-70 transition-colors"
//                           style={{ color: COLOR.primary }}>
//                           Join Meeting <ExternalLink className="w-3 h-3" />
//                         </a>
//                       </DetailRow>
...
//                       className="flex items-center justify-center gap-2 w-full py-2.5 rounded-lg font-semibold text-sm text-white bg-[#1a1a2e] transition-all">
//                       <CreditCard className="w-4 h-4" /> Complete Payment
//                     </button>
//                   )}
//                   {selectedBooking.paymentStatus === "Approved" && selectedBooking.meetingLink && (
//                     <a href={selectedBooking.meetingLink} target="_blank" rel="noopener noreferrer"
//                       className="flex items-center justify-center gap-2 w-full py-2.5 rounded-lg font-semibold text-sm text-white hover:opacity-90 transition-colors"
//                       style={{ backgroundColor: COLOR.dark }}>
//                       <Video className="w-4 h-4" /> Join Meeting
//                     </a>
//                   )}
...
//                       className="w-full py-2.5 rounded-lg font-semibold text-sm text-white hover:opacity-90 transition-colors"
//                       style={{ backgroundColor: COLOR.dark }}>
//                       Reschedule
//                     </button>
//                   )}
...
//                       className="w-full py-2.5 rounded-lg font-semibold text-sm hover:opacity-90 transition-colors border"
...
//               <>
//                 <div className="flex items-center gap-3 px-5 py-4 border-b border-slate-100 flex-shrink-0">
//                   <button onClick={() => setPanelView("details")} className="hover:text-blue-600 transition-colors">
//                     <ArrowLeft className="w-5 h-5" />
//                   </button>
//                   <h2 className="text-sm font-semibold text-slate-900">Cancel Booking</h2>
//                   <button onClick={closePanel} className="ml-auto hover:text-blue-600 transition-colors">
//                     <X className="w-5 h-5" />
//                   </button>
//                 </div>
...
//                   <button
//                     onClick={() => { setPanelView("details"); setCancelReason(""); }}
//                     className="flex-1 py-2.5 rounded-lg font-medium text-sm border hover:opacity-90 transition-colors"
//                     style={{ backgroundColor: "white", color: COLOR.dark, borderColor: COLOR.border }}>
//                     Keep Booking
//                   </button>
//                   <button
//                     onClick={handleCancelBooking}
//                     disabled={isCancelling}
//                     className="flex-1 py-2.5 rounded-lg font-semibold text-sm text-white hover:opacity-90 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
//                     style={{ backgroundColor: COLOR.dark }}>
//                     {isCancelling
//                       ? <span className="flex items-center justify-center gap-2"><Loader2 className="w-4 h-4 animate-spin" /> Cancelling…</span>
//                       : "Yes, Cancel"}
