...
//     const canCancel = sub.status === "active" || (sub.status === "onprocess" && !sub.payment_done); return (
...
// ── Subscription Card ─────────────────────────────────────────────────────────
const SubscriptionCard = ({ sub, i, onView, onPay, onCancel }) => {
    // Allow cancel for active+paid OR onprocess/unpaid
    const canCancel = (sub.status === "active" && sub.payment_done) ||
        (sub.status === "onprocess" && !sub.payment_done) ||
        (sub.status === "active" && !sub.payment_done);

    return (
        <div className="sub-card" style={{ background: C.white, borderRadius: 12, border: `1px solid ${C.border}`, overflow: "hidden", display: "flex", flexDirection: "column", fontFamily: FONT, animationDelay: `${i * 70}ms` }}>
            <div style={{ height: 3, background: sub.status === "cancelled" ? C.danger : C.dark, flexShrink: 0 }} />
            <div style={{ padding: "12px", display: "flex", flexDirection: "column", gap: 10, flex: 1 }}>

                {/* Icon + status */}
                <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                    <div style={{ width: 28, height: 28, borderRadius: 7, background: C.dark, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
                        <CreditCard size={13} style={{ color: C.white }} />
                    </div>
                    <StatusBadge status={sub.status} />
                </div>
                ...
                <div style={{ marginTop: "auto", paddingTop: 10, borderTop: `1px solid ${C.border}`, display: "flex", alignItems: "center", justifyContent: "space-between", gap: 6 }}>
                    ...
                    <div style={{ display: "flex", gap: 6, marginLeft: "auto" }}>
                        {canCancel && (
                            <button className="cancel-btn" onClick={() => onCancel(sub)} style={{ background: "#fff", color: C.danger, border: `1px solid #fecaca`, borderRadius: 7, padding: "6px 10px", fontSize: 11, fontWeight: 700, cursor: "pointer", fontFamily: FONT, transition: "all 0.15s" }}>
                                Cancel
                            </button>
                        )}
                        <button className="pay-btn" onClick={() => onView(sub)} style={{ background: C.dark, color: C.white, border: "none", borderRadius: 7, padding: "6px 12px", fontSize: 11, fontWeight: 700, cursor: "pointer", fontFamily: FONT, transition: "all 0.2s" }}>
                            View Details
                        </button>
                    </div>
                </div>
                ...
                {sub.payment_status === "pending" && (
                    <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 8, flexWrap: "wrap", border: `1px solid ${C.border}`, borderRadius: 7, padding: "8px 10px", background: "#fafbfc" }}>
                        <span style={{ display: "flex", alignItems: "center", gap: 5, fontSize: 11, fontWeight: 600, color: C.text, fontFamily: FONT }}>
                            <AlertCircle size={11} style={{ color: C.blue }} />
                            Payment Incomplete
                        </span>
                        <button className="pay-btn" onClick={() => onPay(sub)} style={{ background: C.dark, color: C.white, border: "none", borderRadius: 7, padding: "6px 12px", fontSize: 11, fontWeight: 700, cursor: "pointer", fontFamily: FONT, transition: "all 0.2s", whiteSpace: "nowrap" }}>
                            Pay Now
                        </button>
                    </div>
                )}
...
const CancelModal = ({ sub, onClose, onConfirmed }) => {
    ...
    return (
        <div onClick={onClose} style={{ position: "fixed", inset: 0, zIndex: 60, display: "flex", alignItems: "center", justifyContent: "center", background: "rgba(0,0,0,0.45)", backdropFilter: "blur(4px)", padding: 16, animation: "fadeIn 0.15s ease" }}>
            <div onClick={e => e.stopPropagation()} style={{ width: "100%", maxWidth: 420, background: C.white, borderRadius: 14, border: `1px solid ${C.border}`, boxShadow: "0 20px 60px rgba(0,0,0,0.18)", overflow: "hidden", animation: "slideUp 0.2s ease", fontFamily: FONT }}>

                {/* Header */}
                <div style={{ background: step === "done" ? "#16a34a" : C.danger, padding: "16px 18px", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                        {step === "done"
                            ? <CheckCircle2 size={18} style={{ color: C.white }} />
                            : <ShieldAlert size={18} style={{ color: C.white }} />
                        }
                        <h2 style={{ fontSize: 15, fontWeight: 700, color: C.white, margin: 0, fontFamily: FONT }}>
                            {step === "done" ? "Subscription Cancelled" : "Cancel Subscription"}
                        </h2>
                    </div>
                    <button onClick={onClose} style={{ width: 28, height: 28, borderRadius: 7, background: "rgba(255,255,255,0.15)", border: "none", color: C.white, display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer" }}>
                        <X size={13} />
                    </button>
                </div>
                ...
                <div style={{ padding: "18px" }}>
                            ...
                            {isUnpaid ? (
                                <>
                                    ...
                                    <div style={{ display: "flex", gap: 8 }}>
                                        ...
                                        <button onClick={handleConfirm} disabled={!agreed || isCancelling} style={{
                                            flex: 1, background: agreed ? C.danger : "#f1f5f9",
                                            color: agreed ? C.white : C.muted,
                                            border: "none", borderRadius: 8, padding: "10px 0",
                                            fontSize: 13, fontWeight: 700,
                                            cursor: agreed && !isCancelling ? "pointer" : "not-allowed",
                                ...
                                <>
                                    ...
                                    <div style={{ background: "#fff7ed", border: "1px solid #fed7aa", borderRadius: 10, padding: "12px 14px", marginBottom: 16 }}>
                                        {[["Days subscribed", `${p.days_since_subscribed} day${p.days_since_subscribed !== 1 ? "s" : ""}`]].map(([lbl, val]) => (
                                            <div key={lbl} style={{ display: "flex", justifyContent: "space-between", marginBottom: 4 }}>
                                                <span style={{ fontSize: 12, color: C.sub, fontFamily: FONT }}>{lbl}</span>
                                                <span style={{ fontSize: 12, fontWeight: 700, color: C.text, fontFamily: FONT }}>{val}</span>
                                            </div>
                    ...
                    {step === "confirm" && p && (
                        <>
                            ...
                            <div style={{ display: "flex", gap: 8 }}>
                                <button onClick={() => setStep("policy")} style={{ flex: 1, background: C.white, color: C.text, border: `1px solid ${C.border}`, borderRadius: 8, padding: "10px 0", fontSize: 13, fontWeight: 700, cursor: "pointer", fontFamily: FONT }}>
                                    ← Back
                                </button>
                                <button onClick={handleConfirm} disabled={isCancelling} style={{
                                    flex: 1, background: C.danger, color: C.white, border: "none", borderRadius: 8,
                                    ...
                                    fontFamily: FONT, opacity: isCancelling ? 0.7 : 1,
                                    display: "flex", alignItems: "center", justifyContent: "center", gap: 6,
                                }}>
                                    {isCancelling && <Loader2 size={13} className="spin-icon" />}
                                    Confirm Cancel
                    ...
                    {step === "done" && done && (
                        <>
                            <div style={{ background: "#f0fdf4", border: "1px solid #bbf7d0", borderRadius: 10, padding: "14px", textAlign: "center", marginBottom: 14 }}>
                                <p style={{ fontSize: 11, color: "#16a34a", fontWeight: 700, letterSpacing: "0.06em", margin: "0 0 4px", fontFamily: FONT }}>
                                    {done.refund_amount > 0 ? "REFUND AMOUNT" : "SUBSCRIPTION CANCELLED"}
                                </p>
                                {done.refund_amount > 0
                                    ? <p style={{ fontSize: 26, fontWeight: 800, color: "#15803d", margin: 0, fontFamily: FONT }}>{money(done.refund_amount)}</p>
                                    : <p style={{ fontSize: 14, fontWeight: 700, color: "#15803d", margin: "4px 0 0", fontFamily: FONT }}>No refund applicable — payment was not made.</p>
                                }
                                {done.completed_sessions != null && (
                                    <p style={{ fontSize: 11, color: C.muted, margin: "6px 0 0", fontFamily: FONT }}>
                                        {done.completed_sessions} of {done.total_sessions} sessions used
                                        {done.refund_percentage != null ? ` · ${done.refund_percentage}% policy applied` : ""}
                                    </p>
                                )}
                            </div>
                            <button onClick={() => { onConfirmed(); onClose(); }} style={{ width: "100%", background: C.dark, color: C.white, border: "none", borderRadius: 8, padding: "10px 0", fontSize: 13, fontWeight: 700, cursor: "pointer", fontFamily: FONT }}>
                                Done
                            </button>
                        </>
                    )}
...
const Subscriptionplan = () => {
    ...
    return (
        <>
            ...
            {/* View Details Modal */}
            {selected && (
                <div onClick={() => setSelected(null)} style={{ position: "fixed", inset: 0, zIndex: 50, display: "flex", alignItems: "center", justifyContent: "center", background: "rgba(0,0,0,0.4)", backdropFilter: "blur(4px)", padding: 16, animation: "fadeIn 0.15s ease" }}>
                    <div onClick={e => e.stopPropagation()} style={{ width: "100%", maxWidth: 440, background: C.white, borderRadius: 14, boxShadow: "0 20px 60px rgba(0,0,0,0.16)", overflow: "hidden", animation: "slideUp 0.2s ease", fontFamily: FONT, maxHeight: "90vh", overflowY: "auto" }}>

                        <div style={{ background: C.dark, padding: "16px 18px", position: "sticky", top: 0, zIndex: 1 }}>
                            <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between" }}>
                                <div>
                                    <p style={{ fontSize: 9, fontWeight: 700, color: "rgba(255,255,255,0.5)", letterSpacing: "0.2em", margin: "0 0 4px", fontFamily: FONT }}>Subscription Details</p>
                                    <h2 style={{ fontSize: 16, fontWeight: 700, color: C.white, margin: "0 0 3px", fontFamily: FONT }}>{planLabel(selected.plan_type)} Plan</h2>
                                </div>
                                <button onClick={() => setSelected(null)} style={{ width: 30, height: 30, borderRadius: 7, color: "rgba(255,255,255,0.7)", display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer" }}>
                                    <X size={24} />
                                </button>
                            </div>
                        </div>
                        ...
                        <div style={{ padding: "16px 18px" }}>
                            ...
                            {selected.status === "cancelled" && (
                                <div style={{ marginBottom: 14, background: "#fef2f2", border: "1px solid #fecaca", borderRadius: 8, padding: "10px 12px" }}>
                                    <div style={{ display: "flex", alignItems: "center", gap: 7, marginBottom: 4 }}>
                                        <XCircle size={13} style={{ color: C.danger }} />
                                        <p style={{ fontSize: 12, fontWeight: 700, color: C.danger, margin: 0, fontFamily: FONT }}>Cancelled on {fmt(selected.cancelled_at)}</p>
                                    </div>
                                    <p style={{ fontSize: 11, color: "#991b1b", margin: 0, fontFamily: FONT }}>
                                        Refund: {money(selected.refund_amount)} · {selected.sessions_completed_at_cancellation} sessions used
                                    </p>
                            ...
                            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
                                {[
                                    ...
                                    <div key={label} style={{ borderRadius: 8, padding: "10px 12px", background: highlight ? "#f0f9ff" : "#f8fafc", border: `1px solid ${highlight ? "#bae6fd" : C.border}` }}>
                                        <p style={{ fontSize: 9, fontWeight: 700, color: highlight ? C.blue : C.muted, letterSpacing: "0.08em", margin: "0 0 3px", fontFamily: FONT }}>{label}</p>
                                        <p style={{ fontSize: 13, fontWeight: 700, color: C.text, margin: 0, fontFamily: FONT, lineHeight: 1.3 }}>{value}</p>
                                    </div>
                                ))}
                            ...
                            <div style={{ display: "flex", gap: 8, marginTop: 16 }}>
                                {selected.payment_status === "pending" && (
                                    <button className="pay-btn" onClick={() => handleCompletePayment(selected)} style={{ flex: 1, background: C.dark, color: C.white, border: "none", borderRadius: 8, padding: "10px 0", fontSize: 13, fontWeight: 700, cursor: "pointer", fontFamily: FONT, transition: "all 0.2s" }}>
                                        Pay Now
                                    </button>
                                )}
                                {selected.status === "active" && selected.payment_done && (
                                    <button onClick={() => { setSelected(null); setCancelTarget(selected); }} style={{ flex: 1, background: C.white, color: C.danger, border: `1px solid #fecaca`, borderRadius: 8, padding: "10px 0", fontSize: 13, fontWeight: 700, cursor: "pointer", fontFamily: FONT }}>
                                        Cancel Plan
                                    </button>
                                )}
                                <button onClick={() => setSelected(null)} style={{ flex: 1, background: C.dark, color: C.white, border: `1px solid ${C.dark}`, borderRadius: 8, padding: "10px 0", fontSize: 13, fontWeight: 700, cursor: "pointer", fontFamily: FONT, transition: "all 0.15s" }}>
                                    Close
                                </button>
                            </div>
                        </div>
