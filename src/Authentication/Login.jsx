...
//       showToast.error(error?.data?.message || 'Failed to reset password. Please try again.');
//     }
//   };
...
//         showToast.error('Please enter your phone.');
//         return;
//       }
...
//       }
//       if (!otpSent) {
//         showToast.warning('Please send OTP first.');
//         return;
//       }
//       if (!formData.otp.trim()) {
...
//             email: formData.email,
//             password: formData.password,
//             role: 1 // Default to mentee role for signup
//           }).unwrap();
...
//           // Navigate based on role or mentor booking
//           if (mentorId) {
//             navigate(`/book-session?mentorId=${mentorId}`);
//           } else {
//             if (loginResponse.data.role === 2) {
//               navigate('/mentor/dashboard');
//             } else if (loginResponse.data.role === 1) {
...
//           // If auto login fails, still show success and switch to login
//           setFormData({
//             name: '',
//             email: formData.email,
//             phone: '',
...
const LoginPage = () => {
  ...
  const handleResetPassword = async () => {
    ...
    try {
      await verifyOtp({ email: formData.email, otp: Number(formData.otp), password: formData.newPassword }).unwrap();
      showToast.success('Password reset successful! You can now login with your new password.');
