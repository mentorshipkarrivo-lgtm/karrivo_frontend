...
// /* ─── Helper: check for a pending booking and navigate back if found ─── */
// const redirectAfterLogin = (navigate, fallbackRole) => {

//   const pendingMentorId = localStorage.getItem("pendingBookingMentorId");

//   console.log(pendingMentorId,"pendingMentorId23fr")
//   if (pendingMentorId) {
//     localStorage.removeItem("pendingBookingMentorId");
//     navigate(`/mentor-profile/${pendingMentorId}`, { replace: true });
//   }
//   // const pendingMentorId = localStorage.getItem("pendingBookingMentorId");
//   // if (pendingMentorId) {
//   //   localStorage.removeItem("pendingBookingMentorId");
...
//   if (fallbackRole === 2) navigate('/mentor/dashboard', { replace: true });
//   else if (fallbackRole === 1) navigate('/mentee/bookings', { replace: true });
//   else navigate('/dashboard', { replace: true });
// };
...
//     const userRole = localStorage.getItem("userRole");
//     if (authToken) {
//       redirectAfterLogin(navigate, Number(userRole));
//     }
//   }, [navigate, mentorId]);
...
//       }
//       showToast.success('Google login successful!');
//       redirectAfterLogin(navigate, response.data.role);
//     } catch (error) {
//       showToast.error(error?.data?.message || 'Google login failed.');
//     }
//   };
...
//             localStorage.setItem('userName', loginResponse.data.name);
//           }
//           // ── NEW: check for pending booking before defaulting to dashboard ──
//           redirectAfterLogin(navigate, loginResponse.data.role);
//         } catch {
//           setFormData({ name: '', email: formData.email, phone: '', countryCode: '+91', password: '', confirmPassword: '', otp: '', newPassword: '' });
//           setOtpSent(false);
//           setIsLogin(true);
